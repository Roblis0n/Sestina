...
export interface ResearchRoomProvider {
  ...
  analyze(input: ResearchRoomProviderInput, options: { readonly signal: AbortSignal }): Promise<unknown>;
}
...
export interface ResearchRoomProviderInput {
  ...
  readonly context: {
    ...
    readonly acceptedDecisions: readonly { readonly id: string; readonly statement: string; readonly status: "accepted" | "frozen" }[];
    readonly openIssues: readonly { readonly id: string; readonly kind: string; readonly summary: string; readonly status: string }[];
    readonly currentEpisode?: { readonly id: string; readonly status: string };
    readonly singleSuggestion: string;
  };
}
...
export class ResearchRoomService {
  ...
  prepare(input: PrepareResearchRoomReviewInput): CoreResult<PreparedResearchRoomReview> {
    ...
    const context = {
      researchQuestion: bundle.value.briefVersion.projectQuestion,
      currentTask: bundle.value.briefVersion.currentTask,
      fixedDecisions: bundle.value.briefVersion.fixedDecisions.map((item) => item.statement),
      acceptedDecisions: bundle.value.decisions.map((item) => ({ id: item.id, statement: item.statement, status: item.status as "accepted" | "frozen" })),
      openIssues: bundle.value.issues.map((item) => ({ id: item.id, kind: item.kind, summary: item.summary, status: item.status })),
      ...(bundle.value.episode ? { currentEpisode: { id: bundle.value.episode.id, status: bundle.value.episode.status } } : {}),
      singleSuggestion: input.suggestion.trim(),
    };
    const contextHash = hash(context); const suggestionHash = hash(input.suggestion.trim()); const bindingHash = hash(bundle.value.state.stateBinding);
    if (!contextHash.ok || !suggestionHash.ok || !bindingHash.ok) return coreErr("infrastructure_failure");
    const fields: ResearchRoomContextManifest["fields"] = Object.freeze([
      ...
      { category: "open_issues", source: "versioned_research_state", sensitivity: "research_state", included: true, truncated: false },
    ...
    const manifest: ResearchRoomContextManifest = Object.freeze({
      ...
      networkRequired: this.provider?.networkAccess === "external", sendStatus: "not_sent", networkUsed: false,
    ...
    const providerInput: ResearchRoomProviderInput = Object.freeze({ schemaVersion: "1.0.0", manifest, context: Object.freeze(context) });
    this.#pending.set(reviewId, { reviewId, suggestion: input.suggestion.trim(), evidenceClass: evidence.value, manifest, manifestHash: manifestHash.value, confirmationNonce, stateBinding: bundle.value.state.stateBinding, providerInput });
  ...
  async analyze(input: { readonly reviewId: string; readonly confirmationNonce: string; readonly manifestHash: string }): Promise<CoreResult<AnalyzedResearchRoomReview>> {
    ...
    if (this.provider !== undefined) {
      invoked = true; networkUsed = this.provider.networkAccess === "external";
      const controller = new AbortController(); const timer = setTimeout(() => { controller.abort(); }, this.timeoutMs);
      try {
        const response = await Promise.race([
          this.provider.analyze(pending.providerInput, { signal: controller.signal }),
          new Promise<never>((_resolve, reject) => { controller.signal.addEventListener("abort", () => { reject(new Error("provider_timeout")); }, { once: true }); }),
        ]);
        const parsed = parseResearchRoomAnalysisPayload(response, { providerResponse: true });
        if (parsed.ok) { providerStatus = "semantic_ready"; ledgerOnlyReason = undefined; analysis = parsed.value; }
        else ledgerOnlyReason = "provider_invalid_response";
      } catch (error) {
        ledgerOnlyReason = controller.signal.aborted || (error instanceof Error && error.message === "provider_timeout") ? "provider_timeout" : "provider_failed";
    ...
    const entry: AnalyzedEntry = Object.freeze({ reviewId: pending.reviewId, providerStatus, ...(ledgerOnlyReason ? { ledgerOnlyReason } : {}), manifest, analysis, authorityNonce: nonce(), stateBinding: pending.stateBinding, suggestionHash: pending.manifest.suggestionHash, evidenceClass: pending.evidenceClass });
  ...
  private readState(projectId: string): CoreResult<StateBundle> {
    ...
    const issues = issuePage.value.items.filter((item) => !["resolved", "suppressed", "waived"].includes(item.status)).sort((a, b) => a.id.localeCompare(b.id));
    ...
    return coreOk({ state, brief: active.brief, briefVersion: active.version, decisions, issues, ...(episode ? { episode } : {}) });
  }
}
