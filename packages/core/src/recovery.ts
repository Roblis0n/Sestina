...
const RESTORE_JOURNAL = ".managed-restore.json";
const RESTORE_FILES = [
  DATABASE_FILE,
  `${DATABASE_FILE}-wal`,
  `${DATABASE_FILE}-shm`,
...
export interface RecoveryFaultInjection {
  ...
  readonly beforeRollbackCleanup?: () => void | Promise<void>;
}

export interface ProjectRecoveryOptions {
  readonly projectRoot: string;
  readonly kernelRecovery?: boolean;
...
export interface ProjectRecoveryStatus {
  ...
  readonly schema: {
    readonly status:
      "recognized" | "too_old" | "too_new" | "migration_failed" | "unavailable";
...
export interface ProjectStateRestoreResult {
  ...
  readonly rollback: {
    ...
    readonly currentStatePreserved: true;
  };
  readonly networkUsed: false;
}
...
interface BundleManifest {
  ...
  readonly bindingHash: string;
  readonly kernelIdentity?: {
    readonly revision: number;
    readonly canonicalHash: string;
    readonly privacyHash: string;
  };
}
...
function fail(code: CoreErrorCode): never {
  throw new RecoveryFailure(code);
}

function resultFor<T>(error: unknown): CoreResult<T> {
  return coreErr(
    error instanceof RecoveryFailure ? error.code : "infrastructure_failure",
  );
}
...
function isSafeSize(value: unknown, maximum: number): value is number {
  return (
...
function bindingHash(
  ...
  return sha256(
    JSON.stringify({
      ...
      ...(input.kernelIdentity ? { kernelIdentity: input.kernelIdentity } : {}),
    }),
  );
}
...
function parseManifest(value: unknown, expectedId: string): BundleManifest {
  if (
    ...
    !sameKeys(
      value,
      value.databaseSchemaVersion === 25
        ? [...MANIFEST_KEYS, "kernelIdentity"]
        : MANIFEST_KEYS,
    )
  )
    fail("state_conflict");
  ...
  if (!isRecord(database) || !sameKeys(database, ["sha256", "sizeBytes"]))
    fail("state_conflict");
  if (!isRecord(brief) || !sameKeys(brief, ["sha256", "sizeBytes"]))
    fail("state_conflict");
  if (
    value.schemaVersion !== MANIFEST_SCHEMA_VERSION ||
    value.backupId !== expectedId ||
    !MANAGED_ID.test(expectedId)
  )
    fail("state_conflict");
  if (
    value.kind !== "manual" &&
    value.kind !== "pre_restore" &&
    value.kind !== "pre_upgrade"
  )
    fail("state_conflict");
  if (
    (value.kind === "manual") !== MANUAL_ID.test(expectedId) ||
    (value.kind === "pre_upgrade") !== PRE_UPGRADE_ID.test(expectedId)
  )
    fail("state_conflict");
  if (
  ...
  if (
    ...
    Number(value.databaseSchemaVersion) <
      SESTINA_RELEASE_CONTRACT.supportedSchemaMinimum ||
    (Number(value.databaseSchemaVersion) > SCHEMA_VERSION &&
      value.databaseSchemaVersion !== 25)
  )
    fail("state_conflict");
  if (value.databaseSchemaVersion === 25) {
    const identity = value.kernelIdentity;
    if (
      !isRecord(identity) ||
      !sameKeys(identity, ["revision", "canonicalHash", "privacyHash"]) ||
      !Number.isSafeInteger(identity.revision) ||
      Number(identity.revision) < 1 ||
      !isHash(identity.canonicalHash) ||
      !isHash(identity.privacyHash)
    )
      fail("state_conflict");
  }
  for (const name of [
    ...
    if (typeof field !== "string" || field.length < 3 || field.length > 256)
      fail("state_conflict");
  }
  if (
    !isHash(database.sha256) ||
    !isSafeSize(database.sizeBytes, MAX_DATABASE_BYTES)
  )
    fail("state_conflict");
  if (!isHash(brief.sha256) || !isSafeSize(brief.sizeBytes, MAX_BRIEF_BYTES))
    fail("state_conflict");
...
async function pathsFor(rawProjectRoot: string): Promise<ProjectPaths> {
  if (typeof rawProjectRoot !== "string" || rawProjectRoot.trim().length === 0)
    fail("invalid_input");
  const projectRoot = await realpath(resolve(rawProjectRoot)).catch(() =>
    fail("not_found"),
  );
  ...
  if (
    ...
    fail("not_found");
  const dataReal = await realpath(dataRoot).catch(() =>
  ...
  const paths = {
    projectRoot,
    dataRoot: dataReal,
    databasePath: join(dataReal, DATABASE_FILE),
    briefPath: join(dataReal, BRIEF_FILE),
    manualRoot: join(dataReal, "backups", "manual"),
    forensicRoot: join(dataReal, "backups", "forensic"),
  };
  for (const candidate of [
    join(dataReal, "backups"),
    paths.manualRoot,
    paths.forensicRoot,
  ]) {
    ...
    if (candidateRel === ".." || candidateRel.startsWith(`..${sep}`))
      fail("invalid_input");
  }
  return paths;
}
...
async function assertManagedDirectory(
  ...
  const manualReal = await realpath(manualRoot).catch(() =>
    fail("infrastructure_failure"),
  );
...
async function validateDatabaseBriefBinding(
  databasePath: string,
  briefText: string,
  manifest: BundleManifest,
): Promise<void> {
  if (manifest.databaseSchemaVersion === 25) {
    const binding = await kernelBackupBinding(databasePath, briefText);
    if (
      binding.projectId !== manifest.projectId ||
      binding.activeBriefId !== manifest.activeBriefId ||
      binding.activeBriefVersionId !== manifest.activeBriefVersionId ||
      kernelHash(binding.kernelIdentity) !== kernelHash(manifest.kernelIdentity)
    )
      fail("state_conflict");
    return;
  }
  const validationRoot = await mkdtemp(
    join(tmpdir(), "sestina-bundle-validation-"),
  );
  const validationDatabase = join(validationRoot, DATABASE_FILE);
  try {
    await copyFile(databasePath, validationDatabase);
    const database = await openDatabase({
      path: validationDatabase,
      readOnly: true,
      immutable: true,
    }).catch(() => fail("state_conflict"));
    try {
      const integrityRows = database.raw
        .prepare("PRAGMA integrity_check")
        .all() as Record<string, unknown>[];
      if (
        integrityRows.length === 0 ||
        !integrityRows.every((row) =>
          Object.values(row).every((value) => value === "ok"),
        )
      )
        fail("state_conflict");
      if (readSchemaVersion(database) !== manifest.databaseSchemaVersion)
        fail("state_conflict");
    } finally {
      database.close();
    }
    ...
    try {
      ...
      if (
        ...
        projects.value[0]?.id !== manifest.projectId
      )
        fail("state_conflict");
      const projection = opened.value.getActiveBriefProjection(
        manifest.projectId,
      );
      if (!projection.ok || projection.value === undefined)
        fail("state_conflict");
      if (
        projection.value.briefId !== manifest.activeBriefId ||
        projection.value.versionId !== manifest.activeBriefVersionId ||
        projection.value.yaml !== briefText
      )
        fail("state_conflict");
    } finally {
      opened.value.close();
...
/** Read the existing Kernel validator; backups do not implement research rules. */
async function kernelBackupBinding(
  databasePath: string,
  briefText: string,
  immutable = true,
) {
  ...
  try {
    ...
    if (projects.length !== 1 || !projectId) fail("state_conflict");
    validateKernelDatabase(db, projectId, briefText, true);
    const head = readKernelSnapshot(db, projectId).head;
    const brief = db.get<{ brief_id: string; current_version_id: string }>(
      "SELECT brief_id,current_version_id FROM research_briefs WHERE project_id=? AND current_version_id IS NOT NULL ORDER BY brief_id LIMIT 1",
      projectId,
    );
    const privacyHash = kernelHash(
      db.all(
        "SELECT * FROM research_privacy_redactions WHERE project_id=? ORDER BY redaction_id",
        projectId,
      ),
    );
    return {
      projectId,
      activeBriefId: brief?.brief_id ?? "none",
      activeBriefVersionId: brief?.current_version_id ?? "none",
      kernelIdentity: {
        revision: head.revision,
        canonicalHash: head.canonicalHash,
        privacyHash,
      },
    };
  } finally {
...
async function validateBundle(
  ...
  await regularFile(join(directory, DATABASE_HASH_FILE), 128);
  await regularFile(join(directory, BRIEF_HASH_FILE), 128);
  const databaseStat = await regularFile(databasePath, MAX_DATABASE_BYTES);
  const briefStat = await regularFile(briefPath, MAX_BRIEF_BYTES);
  const manifestBytes = await readFile(manifestPath, "utf8");
  if (Buffer.byteLength(manifestBytes) !== manifestStat.size)
    fail("state_conflict");
  let parsed: unknown;
  try {
  ...
  const databaseHash = await hashFile(databasePath).catch(() =>
    fail("state_conflict"),
  );
  const briefBuffer = await readFile(briefPath);
  const briefHash = sha256(briefBuffer);
  const databaseSidecar = await readFile(
    join(directory, DATABASE_HASH_FILE),
    "utf8",
  );
  ...
  if (
    ...
    databaseSidecar !== `${databaseHash}\n`
  )
    fail("state_conflict");
  if (
    briefHash !== manifest.brief.sha256 ||
    briefStat.size !== manifest.brief.sizeBytes ||
    briefSidecar !== `${briefHash}\n`
  )
    fail("state_conflict");
  await validateDatabaseBriefBinding(
    databasePath,
    briefBuffer.toString("utf8"),
    manifest,
  );
...
async function buildBundle(
  ...
  const backupId = timestampId(
    kind === "manual" ? "bkp" : kind === "pre_restore" ? "pre" : "upg",
  );
  ...
  try {
    assertInsideRoot(
      paths.dataRoot,
      paths.manualRoot,
      "manual recovery directory",
    );
  } catch {
    fail("invalid_input");
  }
  const manualStat = await lstat(paths.manualRoot);
  if (!manualStat.isDirectory() || manualStat.isSymbolicLink())
    fail("invalid_input");
  const temporaryDirectory = join(
    paths.manualRoot,
    `.tmp-${backupId}-${randomBytes(4).toString("hex")}`,
  );
  const destination = join(paths.manualRoot, backupId);
  let published = false;
  let sourceTemporary: string | undefined, bindingTemporary: string | undefined;
  ...
  try {
    ...
    const sourceCopy = await snapshotCurrentToTemporary(paths);
    sourceTemporary = sourceCopy.directory;
    if (!sourceCopy.databasePath) fail("state_conflict");
    const source = await openDatabase({
      path: sourceCopy.databasePath,
      readOnly: true,
    }).catch(() => fail("state_conflict"));
    let backup;
    try {
      backup = await backupDatabase(source, {
      ...
      source.close();
      await rm(sourceCopy.directory, { recursive: true, force: true });
    }
    await rm(`${backup.path}-wal`, { force: true });
    await rm(`${backup.path}-shm`, { force: true });
    ...
    await writeFile(
      join(temporaryDirectory, DATABASE_HASH_FILE),
      `${backup.hash}\n`,
      { encoding: "utf8", flag: "wx" },
    );
    await writeFile(
      join(temporaryDirectory, BRIEF_HASH_FILE),
      `${briefDigest}\n`,
      { encoding: "utf8", flag: "wx" },
    );

    const bindingRoot = await mkdtemp(
      join(tmpdir(), "sestina-backup-binding-"),
    );
    bindingTemporary = bindingRoot;
    const bindingDatabase = join(bindingRoot, DATABASE_FILE);
    await copyFile(databasePath, bindingDatabase);
    const kernelBinding =
      backup.version === 25
        ? await kernelBackupBinding(
            bindingDatabase,
            briefBuffer.toString("utf8"),
          )
        : undefined;
    const opened = kernelBinding
      ? undefined
      : await openSestina({
          databasePath: bindingDatabase,
          readOnly: true,
          immutable: true,
        });
    if (opened && !opened.ok) {
    ...
    let activeBriefId: string;
    let activeBriefVersionId: string;
    try {
      if (kernelBinding)
        ({ projectId, activeBriefId, activeBriefVersionId } = kernelBinding);
      else {
        if (!opened?.ok) fail("state_conflict");
        const projects = opened.value.listProjects();
        if (
        ...
        if (
          !projection.ok ||
          projection.value?.yaml !== briefBuffer.toString("utf8")
        )
          fail("state_conflict");
        activeBriefId = projection.value.briefId;
        activeBriefVersionId = projection.value.versionId;
      }
    } finally {
    ...
    const manifestBase = {
      ...
      databaseSchemaVersion: backup.version,
      projectId,
      activeBriefId,
      activeBriefVersionId,
      database: { sha256: backup.hash, sizeBytes: backup.sizeBytes },
      brief: { sha256: briefDigest, sizeBytes: briefBuffer.byteLength },
      ...(kernelBinding
        ? { kernelIdentity: kernelBinding.kernelIdentity }
        : {}),
    };
    const manifest: BundleManifest = {
      ...manifestBase,
      bindingHash: bindingHash(manifestBase),
    };
    await writeFile(
    ...
    for (const directory of [sourceTemporary, bindingTemporary])
      if (directory)
...
async function snapshotCurrentToTemporary(
  paths: ProjectPaths,
): Promise<{ directory: string; databasePath?: string; briefText?: string }> {
  ...
  const briefText =
    ...
    briefStat.size <= MAX_BRIEF_BYTES
      ? await readFile(paths.briefPath, "utf8")
      : undefined;
  return {
    directory,
    ...(databaseStat?.isFile()
      ? { databasePath: join(directory, DATABASE_FILE) }
      : {}),
...
> {
  ...
  try {
    if (snapshot.databasePath === undefined)
      return {
        currentState: "recovery_required",
        databaseIntegrity: "missing",
        currentBriefBinding: "unavailable",
        schema: {
          status: "unavailable",
    ...
    if (!checkDatabaseIntegrity(snapshot.databasePath, "full").ok)
      return {
        currentState: "recovery_required",
    ...
    const schemaVersion =
      schemaDatabase === undefined
        ? undefined
        : readSchemaVersion(schemaDatabase);
    const failedVersion = schemaDatabase?.get<{ readonly version: number }>(
      "SELECT version FROM migrations WHERE status = 'failed' ORDER BY version LIMIT 1",
    )?.version;
    schemaDatabase?.close();
    if (
      kernelRecovery &&
      schemaVersion === 25 &&
      snapshot.briefText !== undefined
    ) {
      const binding = await kernelBackupBinding(
    ...
    try {
      ...
      const matched =
        ...
        snapshot.briefText === projection.value.yaml;
      return {
        currentState:
          matched && schema.status === "recognized"
            ? "healthy"
            : "recovery_required",
        databaseIntegrity: "ok",
        currentBriefBinding: matched ? "matched" : "mismatched",
        projectId,
        schema,
      };
    } finally {
...
async function validBackupSummaries(
  paths: ProjectPaths,
): Promise<readonly RecoveryBackupSummary[]> {
  ...
  for (const name of names.filter((item) => MANAGED_ID.test(item)).sort()) {
    try {
      ...
      summaries.push({
        backupId: name,
        kind: MANUAL_ID.test(name)
          ? "manual"
          : PRE_UPGRADE_ID.test(name)
            ? "pre_upgrade"
            : "pre_restore",
        verification: "failed",
        valid: false,
...
export async function inspectProjectRecovery(
  options: ProjectRecoveryOptions,
): Promise<CoreResult<ProjectRecoveryStatus>> {
  try {
    const paths = await pathsFor(options.projectRoot);
    if (
      await lstat(join(paths.dataRoot, RESTORE_JOURNAL)).catch(() => undefined)
    ) {
      return coreOk({
        currentState: "recovery_required",
        databaseIntegrity: "failed",
        ...
        schema: {
          status: "unavailable",
          supportedVersion: options.kernelRecovery ? 25 : SCHEMA_VERSION,
          supportedMinimum: 16,
        },
        restoreAvailable: false,
        interruptedRestore: true,
        backups: await validBackupSummaries(paths),
        networkUsed: false,
      });
    }
    const current = await inspectCurrent(paths, options.kernelRecovery);
    const backups = await validBackupSummaries(paths);
    return coreOk(
      Object.freeze({
        ...current,
        restoreAvailable: backups.some((backup) => backup.valid),
...
async function createProjectStateBackupOfKind(
  options: ProjectRecoveryOptions,
  kind: "manual" | "pre_upgrade",
): Promise<CoreResult<ProjectStateBackupResult>> {
  let guard: MaintenanceGuard | undefined;
  try {
    const paths = await pathsFor(options.projectRoot);
    guard = await MaintenanceGuard.acquire({
      databasePath: paths.databasePath,
      ownerId: `core-backup-${process.pid}`,
      scope: "manual_backup",
      busyTimeoutMs: options.maintenanceBusyTimeoutMs,
    });
    if (
      await lstat(join(paths.dataRoot, RESTORE_JOURNAL)).catch(() => undefined)
    )
    ...
    return coreOk(
      Object.freeze({
        ...
        databaseHash: bundle.manifest.database.sha256,
        briefHash: bundle.manifest.brief.sha256,
...
export async function createProjectStateBackup(
  options: ProjectRecoveryOptions,
): Promise<CoreResult<ProjectStateBackupResult>> {
  return createProjectStateBackupOfKind(options, "manual");
}

export async function createPreUpgradeProjectStateBackup(
  options: ProjectRecoveryOptions,
): Promise<CoreResult<ProjectStateBackupResult>> {
  return createProjectStateBackupOfKind(options, "pre_upgrade");
}

export async function previewProjectStateRestore(
  options: PreviewProjectStateRestoreOptions,
): Promise<CoreResult<ProjectStateRestorePreview>> {
  try {
    const paths = await pathsFor(options.projectRoot);
    if (
      await lstat(join(paths.dataRoot, RESTORE_JOURNAL)).catch(() => undefined)
    ...
    if (options.kernelRecovery) await assertKernelRecoveryTarget(paths, bundle);
    else await assertLegacyRecoveryTarget(paths);
    return coreOk(
      Object.freeze({
        backupId: bundle.manifest.backupId,
        kind: bundle.manifest.kind,
        ...
        networkUsed: false as const,
        ...(options.kernelRecovery
          ? { currentStateHash: await currentProjectStateHash(paths) }
          : {}),
      }),
    );
  } catch (error) {
...
export async function verifyManagedRecoveryBundle(
  projectRoot: string,
  backupId: string,
) {
...
async function assertLegacyRecoveryTarget(paths: ProjectPaths): Promise<void> {
  ...
  if ((await inspectCurrent(paths)).schema.status === "too_new")
    fail("state_conflict");
}

async function assertKernelRecoveryTarget(
  paths: ProjectPaths,
  bundle: ValidBundle,
): Promise<void> {
  if (
    !bundle.manifest.kernelIdentity ||
    bundle.manifest.databaseSchemaVersion !== 25
  )
    fail("state_conflict");
  const snapshot = await snapshotCurrentToTemporary(paths);
  try {
    if (!snapshot.databasePath || snapshot.briefText === undefined)
      fail("state_conflict");
    const binding = await kernelBackupBinding(
      snapshot.databasePath,
      snapshot.briefText,
    ...
    // Unverified/corrupt privacy state cannot authorize resurrecting an old copy.
    if (
      binding.projectId !== bundle.manifest.projectId ||
      binding.kernelIdentity.privacyHash !==
        bundle.manifest.kernelIdentity.privacyHash
    )
      fail("state_conflict");
    const db = await openDatabase({
      path: snapshot.databasePath,
    ...
    try {
      const cleanup = db.get<{ status: string; data: string }>(
        "SELECT status,data FROM research_copy_inventory WHERE project_id=? AND copy_id='kernel-privacy-cleanup'",
        binding.projectId,
      );
      if (cleanup) {
        const saved: unknown = JSON.parse(cleanup.data);
        if (
          !isRecord(saved) ||
          !isRecord(saved.plan) ||
          !Array.isArray(saved.plan.files)
        )
          fail("state_conflict");
        if (
          cleanup.status === "pending" ||
          saved.plan.files.some(
            (file) =>
              !isRecord(file) ||
              typeof file.locationToken !== "string" ||
              file.locationToken.startsWith(
                `backups/manual/${bundle.manifest.backupId}/`,
              ),
          )
        )
          fail("state_conflict");
...
async function preserveForensic(paths: ProjectPaths): Promise<string> {
  const forensicId = timestampId("forensic");
  if (!FORENSIC_ID.test(forensicId)) fail("infrastructure_failure");
  await mkdir(paths.forensicRoot, { recursive: true });
  try {
    assertInsideRoot(
      paths.dataRoot,
      paths.forensicRoot,
      "forensic recovery directory",
    );
  } catch {
    fail("invalid_input");
  }
  const temporary = join(paths.forensicRoot, `.tmp-${forensicId}`);
  const destination = join(paths.forensicRoot, forensicId);
  await mkdir(temporary, { recursive: false });
  try {
    const files: { name: string; sha256: string; sizeBytes: number }[] = [];
    for (const [source, name, maximum] of [
      [paths.databasePath, DATABASE_FILE, MAX_DATABASE_BYTES],
      [`${paths.databasePath}-wal`, `${DATABASE_FILE}-wal`, MAX_DATABASE_BYTES],
      [`${paths.databasePath}-shm`, `${DATABASE_FILE}-shm`, MAX_DATABASE_BYTES],
      [paths.briefPath, BRIEF_FILE, MAX_BRIEF_BYTES],
    ] as const) {
    ...
    await writeFile(
      ...
      `${JSON.stringify({ schemaVersion: MANIFEST_SCHEMA_VERSION, forensicId, kind: "forensic_pre_restore", createdAt: new Date().toISOString(), files }, null, 2)}\n`,
      { encoding: "utf8", flag: "wx" },
    );
...
async function validateStagedPair(
  databasePath: string,
  briefPath: string,
  manifest: BundleManifest,
): Promise<void> {
  const databaseInfo = await regularFile(databasePath, MAX_DATABASE_BYTES);
  const briefInfo = await regularFile(briefPath, MAX_BRIEF_BYTES);
  const databaseDigest = await hashFile(databasePath);
  const briefBuffer = await readFile(briefPath);
  const briefDigest = sha256(briefBuffer);
  if (
    databaseInfo.size !== manifest.database.sizeBytes ||
    databaseDigest !== manifest.database.sha256 ||
    briefInfo.size !== manifest.brief.sizeBytes ||
    briefDigest !== manifest.brief.sha256
  )
    fail("state_conflict");
  await validateDatabaseBriefBinding(
    databasePath,
...
export async function restoreProjectState(
...
): Promise<CoreResult<ProjectStateRestoreResult>> {
  ...
  let stageDatabase: string | undefined;
  let stageBrief: string | undefined;
  const rollback: { source: string; saved: string; moved: boolean }[] = [];
  let databaseInstalled = false;
  let briefInstalled = false;
  let committedAndVerified = false;
  let journalPath: string | undefined;
  try {
    const paths = await pathsFor(options.projectRoot);
    const initial = await validateBundle(paths, options.backupId);
    if (options.kernelRecovery)
      lease = acquireProjectWriteLease(paths.databasePath);
    guard = await MaintenanceGuard.acquire({
      databasePath: paths.databasePath,
      ownerId: `core-restore-${process.pid}`,
      scope: "restore",
      busyTimeoutMs: options.maintenanceBusyTimeoutMs,
    });
    if (
    ...
    if (options.kernelRecovery) {
      if (
        !isHash(options.expectedStateHash) ||
        (await currentProjectStateHash(paths)) !== options.expectedStateHash
      )
        fail("confirmation_binding_mismatch");
      if (
        !isHash(options.expectedManifestHash) ||
        sha256(locked.manifestBytes) !== options.expectedManifestHash
      )
        fail("confirmation_binding_mismatch");
      await assertKernelRecoveryTarget(paths, locked);
    } else await assertLegacyRecoveryTarget(paths);

    let preRestoreBackupId: string;
    let forensicCopyPreserved = false;
    const current = await inspectCurrent(paths, options.kernelRecovery);
    if (current.currentState === "healthy") {
      preRestoreBackupId = (await buildBundle(paths, "pre_restore")).manifest
        .backupId;
    } else {
      preRestoreBackupId = await preserveForensic(paths);
      forensicCopyPreserved = true;
    }

    const nonce = randomBytes(6).toString("hex");
    stageDatabase = join(
      paths.dataRoot,
      `.restore-${nonce}-${DATABASE_FILE}.stage`,
    );
    stageBrief = join(paths.dataRoot, `.restore-${nonce}-${BRIEF_FILE}.stage`);
    await copyFile(locked.databasePath, stageDatabase);
    await copyFile(locked.briefPath, stageBrief);
    ...
    if (
      finalSource.manifestBytes !== locked.manifestBytes ||
      finalSource.manifest.bindingHash !== locked.manifest.bindingHash
    )
      fail("state_conflict");
    if (
      options.kernelRecovery &&
    ...
    if (options.kernelRecovery) {
      ...
      for (const name of RESTORE_FILES)
        files.push({
          name,
          original: await optionalFileFingerprint(
            join(paths.dataRoot, name),
            MAX_DATABASE_BYTES,
          ),
        });
      const path = join(paths.dataRoot, RESTORE_JOURNAL);
      ...
      try {
        await file.writeFile(
          JSON.stringify({
            schemaVersion: "1.0.0",
            nonce,
            backupId: locked.manifest.backupId,
            preRestoreBackupId,
            files,
          }) + "\n",
        );
    ...
    if (journalPath) {
      const previous: unknown = JSON.parse(await readFile(journalPath, "utf8"));
      if (!isRecord(previous)) fail("state_conflict");
      const pending = `${journalPath}.${nonce}.pending`;
      const file = await open(pending, "wx", 0o600);
      try {
        await file.writeFile(JSON.stringify({ ...previous, completed: true }));
        await file.sync();
      } finally {
        await file.close();
      }
    ...
    await options.faultInjection?.beforeRollbackCleanup?.();
    ...
    return coreOk(
      Object.freeze({
        ...
        backupId: locked.manifest.backupId,
        projectId: locked.manifest.projectId,
        preRestoreBackupId,
        forensicCopyPreserved,
        databaseIntegrity: "ok" as const,
        briefBinding: "matched" as const,
        sourceManifestHash: sha256(locked.manifestBytes),
        sourceBindingHash: locked.manifest.bindingHash,
        rollback: Object.freeze({
          performed: false as const,
          currentStatePreserved: true as const,
        }),
        networkUsed: false as const,
      }),
    );
    ...
    try {
      ...
      for (const item of [...rollback].reverse())
        if (item.moved) await rename(item.saved, item.source);
      if (journalPath) {
        await rm(journalPath);
        journalPath = undefined;
      }
    ...
    if (!journalPath && stageDatabase !== undefined)
      await rm(stageDatabase, { force: true }).catch(() => undefined);
    if (!journalPath && stageBrief !== undefined)
      await rm(stageBrief, { force: true }).catch(() => undefined);
    guard?.release();
    lease?.release();
  }
}

/** Explicitly undo an interrupted managed swap, using only the recorded files. */
export async function recoverInterruptedProjectStateRestore(
  options: ProjectRecoveryOptions & { confirmed: boolean },
): Promise<CoreResult<{ recovered: true; networkUsed: false }>> {
  if (!options.confirmed) return coreErr("user_confirmation_required");
  let guard: MaintenanceGuard | undefined;
  let lease: ReturnType<typeof acquireProjectWriteLease> | undefined;
  try {
    ...
    guard = await MaintenanceGuard.acquire({
      databasePath: paths.databasePath,
      ownerId: `core-restore-recovery-${process.pid}`,
      scope: "restore",
      busyTimeoutMs: options.maintenanceBusyTimeoutMs,
    });
    const journalPath = join(paths.dataRoot, RESTORE_JOURNAL);
    await regularFile(journalPath, MAX_MANIFEST_BYTES);
    ...
    if (
      ...
      !/^[0-9a-f]{12}$/.test(record.nonce) ||
      typeof record.backupId !== "string" ||
      typeof record.preRestoreBackupId !== "string" ||
      !Array.isArray(record.files) ||
      record.files.length !== 4
    )
      fail("state_conflict");
    const target = await validateBundle(paths, record.backupId);
    const preserved = await validateBundle(paths, record.preRestoreBackupId);
    if (
      !preserved.manifest.kernelIdentity ||
      preserved.manifest.projectId !== target.manifest.projectId
    )
      fail("state_conflict");
    if (record.completed === true) {
      ...
      for (let index = 0; index < RESTORE_FILES.length; index++) {
        const name = RESTORE_FILES[index],
          item: unknown = record.files[index];
        if (
          !name ||
          !isRecord(item) ||
          !sameKeys(item, ["name", "original"]) ||
          item.name !== name
        )
          fail("state_conflict");
        ...
        if (
          original !== null &&
          (!isRecord(original) ||
            !sameKeys(original, ["sha256", "sizeBytes"]) ||
            !isHash(original.sha256) ||
            !isSafeSize(original.sizeBytes, MAX_DATABASE_BYTES))
        )
          fail("state_conflict");
        const saved = join(paths.dataRoot, `.rollback-${record.nonce}-${name}`);
        const found = await optionalFileFingerprint(saved, MAX_DATABASE_BYTES);
        if (
          found &&
          (!isRecord(original) ||
            found.sha256 !== original.sha256 ||
            found.sizeBytes !== original.sizeBytes)
        )
          fail("state_conflict");
        if (found) cleanup.push(saved);
      }
      for (const path of cleanup) await rm(path);
      await rm(journalPath);
      return coreOk({ recovered: true, networkUsed: false });
    }
    const selections: {
      name: string;
      source: string | null;
    ...
    for (let index = 0; index < RESTORE_FILES.length; index++) {
      const item: unknown = record.files[index];
      const name = RESTORE_FILES[index];
      if (!name) fail("state_conflict");
      if (
        !isRecord(item) ||
        !sameKeys(item, ["name", "original"]) ||
        item.name !== name
      ...
      if (
        ...
        fail("state_conflict");
      const saved = join(paths.dataRoot, `.rollback-${record.nonce}-${name}`);
      const savedHash = await optionalFileFingerprint(
        saved,
        MAX_DATABASE_BYTES,
      );
      const current = join(paths.dataRoot, name);
      const currentHash = await optionalFileFingerprint(
        current,
        MAX_DATABASE_BYTES,
      );
      const targetHash =
        name === DATABASE_FILE
          ? target.manifest.database.sha256
          : name === BRIEF_FILE
            ? target.manifest.brief.sha256
            : undefined;
      if (
        currentHash &&
        currentHash.sha256 !== targetHash &&
        (!isRecord(original) || currentHash.sha256 !== original.sha256)
      )
        fail("state_conflict");
      if (original === null) {
        if (savedHash) fail("state_conflict");
        selections.push({ name, source: null, hash: null });
        continue;
      }
      const expected = original as { sha256: string; sizeBytes: number };
      const source =
        savedHash?.sha256 === expected.sha256 &&
        savedHash.sizeBytes === expected.sizeBytes
          ? saved
          : currentHash?.sha256 === expected.sha256 &&
              currentHash.sizeBytes === expected.sizeBytes
            ? current
            : undefined;
      if (!source) fail("state_conflict");
      selections.push({ name, source, hash: expected.sha256 });
    }
    // Validate the original pair in isolation before touching the interrupted target.
    const directory = await mkdtemp(join(tmpdir(), "sestina-restore-recover-"));
    try {
      for (const item of selections)
        if (item.source)
          await copyFile(item.source, join(directory, item.name));
      const binding = await kernelBackupBinding(
        join(directory, DATABASE_FILE),
        await readFile(join(directory, BRIEF_FILE), "utf8"),
        false,
      );
      if (
        binding.projectId !== preserved.manifest.projectId ||
        kernelHash(binding.kernelIdentity) !==
          kernelHash(preserved.manifest.kernelIdentity)
      )
        fail("state_conflict");
      for (const item of selections) {
        const destination = join(paths.dataRoot, item.name);
        if (item.source && (await hashFile(item.source)) !== item.hash)
          fail("state_conflict");
        if (item.source !== destination) {
          await rm(destination, { force: true });
          if (item.source) await copyFile(item.source, destination);
        }
      }
      for (const item of selections)
        if (
          item.hash &&
          (await hashFile(join(paths.dataRoot, item.name))) !== item.hash
        )
          fail("state_conflict");
      await rm(journalPath);
      for (const name of RESTORE_FILES)
        await rm(join(paths.dataRoot, `.rollback-${record.nonce}-${name}`), {
          force: true,
        });
      for (const name of [DATABASE_FILE, BRIEF_FILE])
        await rm(
          join(paths.dataRoot, `.restore-${record.nonce}-${name}.stage`),
          { force: true },
        );
      return coreOk({ recovered: true, networkUsed: false });
    } finally {
...
async function optionalFileFingerprint(
  path: string,
  maximum: number,
  ...
  return Object.freeze({
    sizeBytes: info.size,
    sha256: await hashFile(path).catch(() => fail("state_conflict")),
  });
}
...
export class ProjectRecoveryConfirmationService {
  ...
  constructor(options: ProjectRecoveryConfirmationServiceOptions = {}) {
    ...
    if (
      ...
      this.#ttlMs < 1_000 ||
      this.#ttlMs > 30 * 60_000
    )
      throw new TypeError("invalid_recovery_confirmation_ttl");
  ...
  #prune(now: number): void {
    ...
    for (const [nonce, record] of this.#pending)
      if (record.expiresAtMs + this.#ttlMs <= now) this.#pending.delete(nonce);
  }

  async prepare(
    input: PrepareProjectStateRestoreInput,
  ): Promise<CoreResult<PreparedProjectStateRestore>> {
    try {
      ...
      if (input.kernelRecovery) await assertKernelRecoveryTarget(paths, bundle);
      const currentState = await inspectCurrent(paths, input.kernelRecovery);
      const manifestHash = sha256(bundle.manifestBytes);
      const projectStateHash = await currentProjectStateHash(paths);
      const stateBinding = sha256(
        JSON.stringify({
      ...
      if (
        !/^[0-9a-f]{64}$/u.test(confirmationNonce) ||
        this.#pending.has(confirmationNonce) ||
        this.#used.has(confirmationNonce)
      )
        fail("infrastructure_failure");
      ...
      this.#pending.set(
        confirmationNonce,
        Object.freeze({
          kernelRecovery: input.kernelRecovery === true,
          canonicalProjectRoot: paths.projectRoot,
          backupId: bundle.manifest.backupId,
  ...
  async execute(
    input: ExecuteProjectStateRestoreInput,
  ): Promise<CoreResult<ExecutedProjectStateRestore>> {
    if (!input.confirmed) return coreErr("user_confirmation_required");
    try {
      if (
        !validBindingText(input.sessionBinding) ||
        !/^[0-9a-f]{64}$/u.test(input.confirmationNonce) ||
        !/^[0-9a-f]{64}$/u.test(input.expectedStateBinding)
      )
        fail("invalid_input");
      const now = this.#now();
      this.#prune(now);
      if (this.#used.has(input.confirmationNonce))
        fail("confirmation_replayed");
      ...
      const paths = await pathsFor(input.projectRoot);
      if (
        paths.projectRoot !== record.canonicalProjectRoot ||
        input.backupId !== record.backupId ||
        sha256(input.sessionBinding) !== record.sessionHash ||
        input.expectedStateBinding !== record.stateBinding
      )
        fail("confirmation_binding_mismatch");
      const bundle = await validateBundle(paths, input.backupId);
      const projectStateHash = await currentProjectStateHash(paths);
      const currentBinding = sha256(
        JSON.stringify({
          canonicalProjectRoot: paths.projectRoot,
          backupId: bundle.manifest.backupId,
          manifestHash: sha256(bundle.manifestBytes),
      ...
      if (
        sha256(bundle.manifestBytes) !== record.manifestHash ||
        currentBinding !== record.stateBinding
      )
        fail("confirmation_binding_mismatch");
      if ((input.kernelRecovery === true) !== record.kernelRecovery)
        fail("confirmation_binding_mismatch");
      const restored = await restoreProjectState({
        projectRoot: paths.projectRoot,
        backupId: input.backupId,
        confirmed: true,
        kernelRecovery: record.kernelRecovery,
        expectedStateHash: projectStateHash,
