...
export const OPENAI_COMPATIBLE_API_KEY_REF =
  "sestina/research-room/openai-compatible/api-key";
export const SECOND_OPINION_OPENAI_COMPATIBLE_API_KEY_REF =
  "sestina/research-room/second-opinion/openai-compatible/api-key";
...
function ipv4Parts(hostname: string): readonly number[] | undefined {
  ...
  return numbers.every((part) => part >= 0 && part <= 255)
    ? numbers
    : undefined;
}
...
function isForbiddenNetworkLiteral(hostname: string): boolean {
  ...
  return mappedIpv4 === undefined
    ? false
    : isForbiddenNetworkLiteral(mappedIpv4);
}
...
export function validateOpenAICompatibleBaseUrl(input: unknown): {
  ...
  if (
    ...
    throw new ProviderSettingsError(
      "invalid_provider_config",
      "Provider URLs cannot contain credentials, query values, or fragments.",
    );
  ...
  if (url.protocol !== "https:" && !(url.protocol === "http:" && local))
    throw new ProviderSettingsError(
      "invalid_provider_config",
      "Only external HTTPS or explicit loopback HTTP is allowed.",
    );
  if (!local && isForbiddenNetworkLiteral(url.hostname))
    throw new ProviderSettingsError(
      "invalid_provider_config",
  ...
  if (url.pathname.includes("//"))
    throw new ProviderSettingsError(
      "invalid_provider_config",
      "The Provider base path is invalid.",
    );
  const pathname =
...
function parseConfig(
  ...
  if (
    ...
    value.family !== "openai_compatible" ||
  ...
  if (
    ...
    (!Number.isSafeInteger(value.maxOutputTokens) ||
      ...
      Number(value.maxOutputTokens) > 65_536)
  )
    return undefined;
  let validated: ReturnType<typeof validateOpenAICompatibleBaseUrl>;
  try {
    validated = validateOpenAICompatibleBaseUrl(value.baseUrl);
  } catch {
    return undefined;
  }
  if (
    validated.baseUrl !== value.baseUrl ||
    validated.locality !== value.locality
  )
    return undefined;
  return Object.freeze({
    schemaVersion: "1.0.0",
...
export function resolveDefaultProviderConfigPath(
  options: DefaultProviderConfigPathOptions = {},
): string {
  const platform = options.platform ?? process.platform;
  const environment = options.environment ?? process.env;
  const homeDirectory = options.homeDirectory ?? homedir();
  if (platform === "win32") {
    const appData = configuredDirectory(
  ...
  const config = configuredDirectory(
    environment.XDG_CONFIG_HOME,
    posix.join(homeDirectory.replaceAll("\\", "/"), ".config"),
  );
  return posix.join(config, "sestina", "provider.json");
}

export function resolveDefaultSecondOpinionProviderConfigPath(
  options: DefaultProviderConfigPathOptions = {},
): string {
  ...
  return Object.freeze({
    ...
    async read(): Promise<OpenAICompatibleProviderConfig | undefined> {
      ...
      try {
        ...
        throw new ProviderSettingsError(
          "provider_config_corrupt",
          "The Provider configuration is corrupt and was preserved.",
        );
      }
...
export class ProviderConfigurationService {
  ...
  async save(
    ...
    const apiKey =
      input.apiKey === undefined ? undefined : cleanText(input.apiKey, 8_192);
    ...
    let secretConfigured = false;
    try {
      secretConfigured = (await this.secrets.describe(this.#secretRef))
        .configured;
    } catch {
      /* handled below */
    }
    if (
      validated.locality === "external" &&
      apiKey === undefined &&
      !secretConfigured
    )
      throw new ProviderSettingsError(
        "provider_key_required",
        "External HTTPS Providers require a securely stored API key.",
      );
    if (apiKey !== undefined) {
      let health: SecretBackendStatus;
      try {
        health = await this.secrets.health();
      } catch {
        health = { available: false, backend: "none" };
      }
      if (!health.available)
        throw new ProviderSettingsError(
          "secure_storage_unavailable",
  ...
  async deleteSecret(): Promise<void> {
    try {
      await this.secrets.delete(this.#secretRef);
    } catch {
      throw new ProviderSettingsError(
        "secure_storage_unavailable",
        "Secure storage is unavailable; the API key was not deleted.",
      );
    }
  }
  ...
  async loadRuntimeSnapshot(): Promise<ProviderRuntimeSnapshot | undefined> {
    ...
    try {
      apiKey = await this.secrets.get(this.#secretRef);
    } catch {
      throw new ProviderSettingsError(
        "secure_storage_unavailable",
        "Secure storage is unavailable; Provider use is blocked.",
      );
    }
    if (config.locality === "external" && apiKey === undefined)
      throw new ProviderSettingsError(
        "provider_key_required",
        "The external Provider key is missing; Provider use is blocked.",
      );
    return Object.freeze({
      config,
