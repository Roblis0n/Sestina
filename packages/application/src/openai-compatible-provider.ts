...
export function createKernelOpenAICompatibleProvider(
  ...
  return Object.freeze({
    identity: Object.freeze({
      ...
      origin: new URL(snapshot.config.baseUrl).origin,
    ...
    async send(
      ...
      if (signal.aborted)
        throw new OpenAICompatibleProviderError("provider_aborted");
      if (Buffer.byteLength(body) > 1_048_576)
        throw new OpenAICompatibleProviderError("provider_invalid_request");
      const endpoint = `${snapshot.config.baseUrl}/chat/completions`;
      assertProviderEndpoint(endpoint, snapshot.config.locality);
      if (fetchImplementation) await beforeWrite?.();
      const response = await (
        fetchImplementation ??
        createPinnedProviderFetch(snapshot.config.locality, beforeWrite)
      )(endpoint, {
        method: "POST",
        body,
        signal,
      ...
      if (!response.ok) {
        ...
        throw new OpenAICompatibleProviderError("provider_http_error");
      }
      const raw = await readBoundedBody(response, 524_288);
      let content: string | undefined;
      try {
        content = extractAssistantContent(JSON.parse(raw));
      } catch {
        throw new OpenAICompatibleProviderError("provider_invalid_response");
      }
      if (!content || Buffer.byteLength(content) > 262_144)
        throw new OpenAICompatibleProviderError("provider_invalid_response");
      return content;
    },
  });
...
const SAFE_MESSAGES: Readonly<
  Record<OpenAICompatibleProviderErrorCode, string>
> = Object.freeze({
  provider_invalid_request: "The locked Semantic Judge request is invalid.",
  provider_configuration_changed:
    "The Provider configuration changed after the manifest was prepared.",
...
export async function testOpenAICompatibleProviderConnection(
  ...
  try {
    ...
    /* Metadata test never retains a response body. */
  }
  if (!response.ok)
    throw new OpenAICompatibleProviderError("provider_http_error");
  return Object.freeze({
    reachable: true,
    requestKind: "metadata_only_no_research_context",
    endpoint,
...
function sameBinding(
  left: ResearchRoomSemanticProviderBinding,
  right: ResearchRoomSemanticProviderBinding,
): boolean {
  return (
    left.id === right.id &&
...
function compileBody(
  ...
  return JSON.stringify({
    ...
    ...(snapshot.config.maxOutputTokens === undefined
      ? {}
      : { max_tokens: snapshot.config.maxOutputTokens }),
  });
}

function compileSecondOpinionBody(
  snapshot: ProviderRuntimeSnapshot,
...
function previewFor(
  snapshot: ProviderRuntimeSnapshot,
  binding: ResearchRoomSemanticProviderBinding,
  request: ResearchRoomSemanticJudgeRequest,
): OpenAICompatibleCallPreview {
  if (!sameBinding(request.provider, binding))
    throw new OpenAICompatibleProviderError("provider_invalid_request");
...
function secondOpinionPreviewFor(
  snapshot: ProviderRuntimeSnapshot,
  binding: ResearchRoomSemanticProviderBinding,
  request: CorrectionAppealSecondOpinionRequest,
...
function validPreview(
  actual: OpenAICompatibleCallPreview,
  expected: OpenAICompatibleCallPreview,
): boolean {
  return (
...
export function createOpenAICompatibleProvider(
  ...
  return Object.freeze({
    ...
    networkAccess:
      snapshot.config.locality === "external"
        ? ("external" as const)
        : ("loopback" as const),
    binding,
    prepare(
    ...
    async analyze(
      ...
      if (!validPreview(preview, expected))
        throw new OpenAICompatibleProviderError("provider_invalid_request");
      if (
        (await options.readCurrentGeneration()) !== snapshot.config.generation
      )
        throw new OpenAICompatibleProviderError(
          "provider_configuration_changed",
        );
      ...
      try {
        ...
        if (abortState.timedOut)
          throw new OpenAICompatibleProviderError("provider_timeout");
        if (abortState.callerAborted)
          throw new OpenAICompatibleProviderError("provider_aborted");
      ...
      let raw: string;
      try {
        raw = await readBoundedBody(response, expected.responseLimitBytes);
      } catch (error) {
      ...
      if (
        content === undefined ||
        Buffer.byteLength(content, "utf8") > expected.responseLimitBytes
      )
        throw new OpenAICompatibleProviderError("provider_invalid_response");
      return content;
    },
...
export function createOpenAICompatibleSecondOpinionProvider(
  ...
  const endpointIdentityHash =
    createCorrectionAppealProviderEndpointIdentityHash(binding);
  if (endpointIdentityHash === undefined)
    throw new OpenAICompatibleProviderError("provider_invalid_request");
  const fetchImplementation = options.fetchImplementation ?? fetch;
  return Object.freeze({
    id: snapshot.config.providerId,
...
function deliberationBody(
  snapshot: ProviderRuntimeSnapshot,
  request: DeliberationParticipantRequest,
): string {
...
function sameDeliberationPreview(
  actual: DeliberationParticipantProviderInput,
  expected: DeliberationParticipantProviderInput,
): boolean {
...
export function createOpenAICompatibleDeliberationParticipant(
  ...
  const runtimeIdentityHash = sha256(
    JSON.stringify({
      providerId: snapshot.config.providerId,
  ...
  return Object.freeze({
    ...
    prepare(
      ...
      if (
        request.participant.providerId !== binding.id ||
        request.participant.model !== binding.model ||
        request.participant.configGeneration !== binding.configGeneration
      )
        throw new OpenAICompatibleProviderError("provider_invalid_request");
    ...
    async analyze(
      request: DeliberationParticipantRequest,
      preview: DeliberationParticipantProviderInput,
      { signal }: { readonly signal: AbortSignal },
    ): Promise<string> {
      const expected = deliberationPreview(snapshot, request);
      if (!sameDeliberationPreview(preview, expected))
        throw new OpenAICompatibleProviderError("provider_invalid_request");
      if (
        (await options.readCurrentGeneration()) !== snapshot.config.generation
      )
