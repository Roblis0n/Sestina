...
export function parseMechanismLink(input: unknown): ResearchResult<MechanismLink> {
  ...
  if (!id.ok || !project.ok || !artifact.ok || !revision.ok || !from.ok || !to.ok || from.value.id === to.value.id || !source.ok || !version.ok) return err(researchError("invalid_mechanism_link"));
  return ok(cloneFrozen({ id: id.value.id, projectId: project.value.id, artifactId: artifact.value.id, revisionId: revision.value.id, fromClaimId: from.value.id, toClaimId: to.value.id, relation: input.relation.trim(), intermediateSteps: steps, source: source.value, version: version.value }));
}
