...
function parseSnapshotStructure(input: unknown): ResearchResult<ResearchSnapshot> {
  ...
  if (episode.value.id !== episodeId.value.id || episode.value.projectId !== projectId.value.id || !["accepted", "rejected", "abandoned"].includes(episode.value.status)) return err(researchError("invalid_research_snapshot"));
