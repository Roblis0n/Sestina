...
function startInput(privateRoot: string, participantCode = "EXT-0001") {
  return {
    ...
    hostEntry: "research_room" as const,
    materialType: "paper" as const,
    consentVersion: PILOT_CONSENT_VERSION,
    consentAcknowledged: true,
    releaseVersion: "0.2.0",
    releaseChannel: "public_preview" as const,
    releaseBuildId: BUILD_ID,
    releasePlatform: "windows_x64" as const,
    distributionSource: "github_release" as const,
    releaseSourceCommit: "2".repeat(40),
    releaseAssetSha256: "3".repeat(64),
    operatingMode: "ledger_only" as const,
...
describe("private Pilot session lifecycle", () => {
  ...
  it("requires exit evidence only on the matching exit checkpoint", async () => {
    ...
    const finished = await finishPilotSession({
      ...
      distribution: {
        download: { outcome: "not_observed", durationMinutes: null },
        checksumVerification: {
          outcome: "not_observed",
          durationMinutes: null,
        },
        extraction: { outcome: "not_observed", durationMinutes: null },
        firstLaunch: { outcome: "not_observed", durationMinutes: null },
        timeToRoomMinutes: null,
        failurePoint: null,
      },
      journey: {
      ...
      preferredEntry: "none",
      desktopNeed: "uncertain",
      desktopSolutionEvidence: "uncertain",
      uiNeed: "uncertain",
      syntheticCaseDiscussion: "undecided",
