...
describe("CorrectionRepository (docs/22 Task 9)", () => {
  ...
  it("markSuperseded updates only superseded_by; the data JSON stays byte-identical", () => {
    ...
    uow.commit((u) => {
      u.corrections.markSuperseded(projectA, older.correctionId, newer.correctionId);
    });
    ...
    expect(readOlder?.supersededBy).toBe(newer.correctionId);
    const readNewer = uow.corrections.get(projectA, newer.correctionId);
    expect(readNewer?.supersededBy).toBeUndefined();

    const olderWithout = { ...older };
    delete (olderWithout as { supersededBy?: string }).supersededBy;
    const readWithout = { ...readOlder };
    delete (readWithout as { supersededBy?: string }).supersededBy;
    expect(readWithout).toEqual(olderWithout);
  });

  it("markSuperseded uses the same error for missing and cross-project ids", () => {
    const correction = makeCorrection(projectA, taskA);
    const uow = createUnitOfWork(db);
    uow.commit((u) => {
      u.corrections.insert(projectA, correction);
    ...
    expectSestinaCode(() => {
      uow.commit((u) => {
        u.corrections.markSuperseded(projectA, generateId(), correction.correctionId);
      });
    }, SestinaErrorCode.contract_not_found);
    expectSestinaCode(() => {
      uow.commit((u) => {
        u.corrections.markSuperseded(projectB, correction.correctionId, correction.correctionId);
      });
    }, SestinaErrorCode.contract_not_found);
  });
  ...
  it("rejects writes outside a transaction", () => {
    ...
    expectSestinaCode(() => {
      uow.corrections.markSuperseded(projectA, correction.correctionId, correction.correctionId);
    }, SestinaErrorCode.internal_error);
    expectSestinaCode(() => {
  ...
  it("a readonly database rejects correction writes and persists nothing", async () => {
    ...
    expectSestinaCode(() => {
      roUow.commit((u) => {
        u.corrections.markSuperseded(projectA, correction.correctionId, correction.correctionId);
      });
    }, SestinaErrorCode.database_readonly);
    expectSestinaCode(() => {
      roUow.commit((u) => {
