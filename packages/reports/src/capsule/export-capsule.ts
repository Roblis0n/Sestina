...
import { parseProjectRelativePath, type CheckerIdentity } from "@sestina/review";
...
export function exportCapsule(input: CapsuleExportInput, options: CapsuleExportOptions = {}): ReportResult<{ readonly capsule: ReviewCapsule; readonly json: string }> {
  ...
  const make = () => ({
    ...
    truncationPolicy: { withinSection: "ascending_id_or_text", overflowDropOrder: ["issues", "decisions", "evidenceBoundaries", "expectedDeltas", "briefExpectedDeltas", "invalidationConditions", "checkerVersions"] },
    omissions: { decisions: omittedDecisions, issues: omittedIssues, evidenceBoundaries: omittedBoundaries, expectedDeltas: omittedDeltas, briefExpectedDeltas: omittedBriefDeltas, invalidationConditions: omittedInvalidationConditions, checkerVersions: omittedCheckerVersions, baselineContent: omittedBaselineContent, candidateContent: omittedCandidateContent },
  ...
  if (utf8ByteLength(json) > maxBytes) {
    ...
    while (utf8ByteLength(json) > maxBytes && (decisions.length + issues.length + boundaries.length + deltas.length + briefDeltas.length + invalidationConditions.length + checkerVersions.length > 0)) {
      ...
      else { checkerVersions = checkerVersions.slice(0, -1); omittedCheckerVersions += 1; }
      body = make(); capsule = withHash(); json = serialize(capsule);
    }
  }
