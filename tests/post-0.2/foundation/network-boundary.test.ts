...
it("G1/G2/G3: migration, no-assessment commit, context, projection and downgrade use zero sockets", async () => {
  ...
  const deny = vi
    ...
    .mockImplementation(function () {
      ...
      throw Error("network_forbidden_by_test");
    });
  try {
    await previewKernelMigration(p.root);
    ...
    try {
      ...
      const object = decision(db, p.entry.projectId),
        review = prepare(uow, p.entry.projectId, "create_decision", [
          { kind: "decision", id: object.id, version: 0 },
        ]);
      value(
        uow.commitCanonical(review.command, (r) => {
          value(r.decisions.create(object));
        }),
      );
      const context = projectKernelContext(
        readKernelSnapshot(db, p.entry.projectId),
        "Synthetic local suggestion.",
      );
      expect(context.projection.projectStateRevision).toBe(2);
      value(
        rebuildKernelProjection(db, p.entry.projectId, "search", (s) => ({
    ...
    await restoreKernelPreMigrationBackup(p.root);
...
export async function controlledSocket(
  mode:
    | "reset_after_body"
    | "timeout_after_body"
    | "redirect"
  ...
  const server = createServer(async (request, response) => {
    ...
    if (mode === "reset_after_body") request.socket.destroy();
    else if (mode === "timeout_after_body") {
      /* Deliberately leave the captured request unanswered until client cancellation. */
    } else if (mode === "redirect") {
      response.writeHead(302, { location: "/must-not-follow" });
      response.end();
    } else if (mode === "oversize") response.end("x".repeat(65537));
    else response.end("{invalid-json");
  });
...
it.each([
  ...
  "G1: controlled %s transport captures exact socket body and never retries",
  async (mode) => {
    const fixture = await controlledSocket(mode),
      body = Buffer.from('{"synthetic":"e\u0301 合成"}');
    try {
      let failure: unknown;
      try {
        ...
        if (bytes.length > 65536) throw Error("response_too_large");
        JSON.parse(Buffer.from(bytes).toString("utf8"));
      } catch (error) {
        failure = error;
      }
      expect(failure).toBeDefined();
      expect(fixture.bodies).toEqual([body]);
    } finally {
      await fixture.close();
    }
  },
