...
let ownsRoot = false;
beforeAll(async () => {
  if (process.env.SESTINA_LEGACY_STATES_DIR) {
    root = resolve(process.env.SESTINA_LEGACY_STATES_DIR);
    return;
  }
  ...
  async (entry) => {
    ...
    try {
      ...
      const command = async (body: unknown) => {
        ...
        return post("/api/kernel/reviews", {
      ...
      try {
        expect(
          (await post("/api/kernel/open", { projectPath: project })).data.ok,
        ).toBe(true);
        const history = (
          await command({
        ...
        expect(history.items[0].legacyPayload).toMatchObject({
          id: entry.objectId,
          status: entry.status,
        });
        const input = {
          ...
          suggestion: "Explicitly continue this synthetic historical record",
        };
        const draft = (await command(input)).data.value;
        expect(draft).toMatchObject({
          status: "draft",
          source: { kind: "legacy_workflow", id: `${kind}:${entry.objectId}` },
          attemptIds: [],
          terminalOutcome: null,
        });
        expect((await command(input)).data.value.id).toBe(draft.id);
        expect(
          (await command({ ...input, suggestion: "Conflicting duplicate" }))
            .data.ok,
        ).toBe(false);
        expect(
          (await command({ action: "brief" })).data.value.projectStateRevision,
        ).toBe(1);
        for (const route of [
          "/api/project/correction-appeals/create",
          "/api/project/deliberation-rooms/create",
          "/api/project/external-app-pilots/create",
        ]) {
          expect(
            await post(route, { projectId: entry.projectId }),
          ).toMatchObject({
            status: 409,
            data: { ok: false, error: { code: "legacy_workflow_read_only" } },
          });
        }
        expect(
          (
