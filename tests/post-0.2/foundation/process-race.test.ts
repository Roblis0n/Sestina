...
it("G3: two processes released together compete for the same Decision and exactly one commits", async () => {
  ...
  try {
    ...
    const store = createResearchStore(db),
      old = value(
        store.decisions.listByScope(p.entry.projectId, undefined, {
          limit: 10,
        }),
      ).items[0]!;
    const uow = createResearchUnitOfWork(db).kernel!,
      files: string[] = [];
    for (const target of ["accepted", "rejected"] as const) {
      const object = value(
        transitionResearchDecision(
          ...
          USER,
          old.version,
          `Synthetic competing ${target} decision.`,
          new FixedClock(at),
        ),
      );
      const prepared = prepare(uow, p.entry.projectId, "create_decision", [
        { kind: "decision", id: old.id, version: old.version },
      ]);
      const file = join(p.root, `${target}.json`);
      await writeFile(
        file,
        JSON.stringify({
    ...
    expect(results.filter((r) => !r.ok)).toEqual([
      {
        ...
        error: {
          code: "stale_revision",
          changedObjects: [
            { kind: "decision", id: old.id, version: old.version + 1 },
          ],
        },
      },
    ]);
    ...
    try {
      ...
      expect(
        reopened.all("SELECT * FROM research_transition_receipts"),
      ).toHaveLength(1);
      expect(
        value(
          createResearchStore(reopened).decisions.getById(
            p.entry.projectId,
            old.id,
          ),
        )?.version,
