...
it("G2: direct in-place target migration refusal does not change old journal mode or database bytes", async () => {
  ...
  try {
    ...
    await expect(
      openDatabase({ path, migrate: { migrations: KERNEL_MIGRATIONS } }),
    ).rejects.toBeDefined();
...
it("G2: an unknown target schema trigger is rejected before any writable open", async () => {
  ...
  try {
    ...
    raw.exec(
      "CREATE TRIGGER foreign_target_trigger AFTER UPDATE ON research_reviews BEGIN SELECT 1; END",
    );
    raw.close();
      ...
      const result = uow.commitCanonical(
        {
          ...
          effectKind: "privacy_redaction",
          previewHash: kernelHash({ id: m.id }),
          objectVersions: [{ kind: "memory", id: m.id, version: m.version }],
          actor: USER,
          authorityCapability: capability,
          publicReason: "Synthetic empty request.",
