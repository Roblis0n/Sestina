...
it.each(["column_version", "orphan_attempt_binding"])(
  "G3: startup rejects %s even when the canonical head has not changed",
  async (mode) => {
    ...
    try {
      ...
      const review = value(
        uow.workflow((r) => r.reviews.create(draft(p.entry.projectId, 1))),
      );
      db.close();
      const raw = new DatabaseSync(join(p.root, ".sestina/state.sqlite"));
      try {
        if (mode === "column_version")
          raw
            .prepare("UPDATE research_reviews SET version=99 WHERE review_id=?")
            .run(review.id);
        else
          raw
            .prepare("UPDATE research_reviews SET data=? WHERE review_id=?")
            .run(
              JSON.stringify({ ...review, attemptIds: [ids.create("rpat_")] }),
              review.id,
            );
      } finally {
        raw.close();
      }
      const accepted = await openKernelProject(p.root).then(
        (opened) => {
          opened.close();
          return true;
        },
