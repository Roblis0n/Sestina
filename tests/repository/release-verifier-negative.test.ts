...
function releaseIdentity() {
  return {
    ...
    nativeSecretBackend: platform.nativeSecretBackend,
  };
}

function dataFor(path: string, packageOverride = packageJson): Buffer {
  if (path.endsWith("/package.json") && path === `${root}/package.json`)
    return Buffer.from(JSON.stringify(packageOverride));
  if (path.endsWith("/RELEASE-IDENTITY.json"))
    return Buffer.from(JSON.stringify(releaseIdentity()));
...
describe("RI-54 release verifier fail-closed negative fixtures", () => {
  it("rejects manifest-version and checksum tampering", () => {
    expect(() => {
      validateReleaseManifest({ ...manifest(), schemaVersion: "1.0.0" });
    }).toThrow(/release_manifest_version_invalid/u);
    expect(() =>
      parseChecksums(`${"0".repeat(63)}x  release-manifest.json\n`),
    ).toThrow(/sha256sums_format_invalid/u);
    expect(
      parseChecksums(`${"a".repeat(64)}  docs/CONSENT.md\n`).get(
        "docs/CONSENT.md",
      ),
    ).toBe("a".repeat(64));
    expect(() => parseChecksums(`${"a".repeat(64)}  ../private.txt\n`)).toThrow(
      /sha256sums_format_invalid/u,
    );
  });
  ...
  it("rejects dependency injection, a broken launcher, and missing co-located MCP", () => {
    expect(() => {
      verifyReleaseBundleEntries(
        entries({
          ...packageJson,
          dependencies: { "@sestina/core": "workspace:*" },
        }),
        manifest(),
      );
    }).toThrow(/release_package_manifest_keys_invalid/u);
    expect(() => {
      verifyReleaseBundleEntries(
        entries({ ...packageJson, scripts: { start: "node src/main.ts" } }),
        manifest(),
      );
    }).toThrow(/release_package_identity_invalid/u);
    const withoutMcp = requiredPaths.filter(
      (path) => path !== `${root}/app/mcp/main.js`,
    );
    expect(() => {
      verifyReleaseBundleEntries(
        entries(packageJson, withoutMcp),
        manifest(withoutMcp),
      );
    }).toThrow(/release_bundle_runtime_missing/u);
  });

  it("rejects source maps, development fixtures, secrets, current Brief state, and databases", () => {
    expect(() => {
      scanReleaseEntries([
        { path: `${root}/app/main.js.map`, data: Buffer.from("map") },
      ]);
    }).toThrow(/forbidden_release_path/u);
    expect(() => {
      scanReleaseEntries([
        { path: `${root}/tests/fixture.ts`, data: Buffer.from("fixture") },
      ]);
    }).toThrow(/forbidden_release_path/u);
    expect(() => {
      scanReleaseEntries([
        {
          path: `${root}/README.md`,
          data: Buffer.from("SESTINA_SYNTHETIC_SECRET_DO_NOT_SHIP"),
        },
      ]);
    }).toThrow(/forbidden_release_content/u);
    expect(() => {
      scanReleaseEntries([
        {
          path: `${root}/.sestina/research-brief.yaml`,
    ...
    expect(() => {
      scanReleaseEntries([
        { path: `${root}/state.sqlite`, data: Buffer.from("SQLite") },
      ]);
    }).toThrow(/forbidden_release_path/u);
  });
});
