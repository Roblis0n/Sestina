...
describe("Research Room public-preview release entrypoints", () => {
  it("exposes product-scoped build, verifier, lifecycle, and no-publish gates", async () => {
    ...
    expect(packageJson.scripts["release:fresh-install"]).toBe(
      "node scripts/run-fresh-install.mjs",
    );
    expect(packageJson.scripts["verify:platform"]).toContain(
      "run-public-platform-gates",
    );
    expect(packageJson.scripts["verify:public"]).toContain(
      "run-public-shared-gates",
    );
    expect(packageJson.scripts["pilot:kit:package"]).toContain(
      "package-pilot-kit",
    );
    expect(packageJson.scripts["public-release:assemble"]).toContain(
      "assemble-public-release",
    );
    expect(packageJson.scripts["public-release:verify"]).toContain(
  ...
  it("makes Research Room the artifact entry and keeps CLI/MCP subordinate", async () => {
    ...
    expect(builder).toContain('repositoryEntry("NOTICE"');
    expect(builder).toContain('repositoryEntry("TRADEMARKS.md"');
