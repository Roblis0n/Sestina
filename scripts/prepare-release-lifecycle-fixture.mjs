...
import {
  RandomIdFactory,
  SystemClock,
} from "../packages/core/src/id-factory.ts";
...
const MODEL = Object.freeze({
  kind: "model",
  actorId: "ri53-lifecycle-fixture",
});
const CURRENT_TASK =
  "Verify Decision, Issue, Episode, Receipt, and Brief continuity without network access.";
...
invariant(
  rootArgument !== undefined &&
    ["upgrade", "migration-failure", "future"].includes(mode),
  "Usage: prepare-release-lifecycle-fixture.mjs --root <empty-directory> --mode <upgrade|migration-failure|future>",
);
...
const database = await openDatabase({
  path: databasePath,
  migrate: { migrations: MIGRATIONS.slice(0, targetSchema) },
});
const core = new SestinaCore(
  database,
  new SystemClock(),
  new RandomIdFactory(),
);
...
const project = valueOf(
  core.initializeProject({
    title: "RI-53 " + mode + " continuity",
    rootPath: ".",
    actor: USER,
  }),
);
const artifact = valueOf(
  core.createArtifactWithInitialRevision({
...
valueOf(
  core.activateBrief({
    ...
    actor: USER,
    projectQuestion:
      "Can a Research Room release preserve canonical local research state through upgrade and reinstall?",
    ...
    forbiddenChanges: [
      {
        target: { kind: "project_path", relativePath: ".sestina" },
        operations: ["delete"],
      },
    ],
    expectedDeltas: [
      {
        statement: "Produce exact lifecycle evidence.",
        scope: {
          target: { kind: "artifact", artifactId: artifact.artifact.id },
          operations: ["rewrite"],
        },
    ...
    evidenceBoundaries: [
      {
        statement: "A local pass does not prove external user value.",
        scope: {
          target: { kind: "project_path", relativePath: "paper" },
          operations: ["rewrite"],
        },
...
const issue = valueOf(
  core.openIssue({
    ...
    summary:
      "A local lifecycle alone cannot establish the independent three-platform gate.",
...
const episode = valueOf(
  core.startRevisionEpisode({
    projectId: project.id,
...
const prepared = valueOf(
  core.prepareResearchRoomReview({
    projectId: project.id,
    suggestion:
      "Retain the explicit recovery boundary and record platform-specific evidence.",
    evidenceClass: "synthetic_fixture",
    countsAsExternalEvidence: false,
  }),
);
const analyzed = valueOf(
...
const receipt = valueOf(
  core.commitResearchRoomDisposition({
    ...
    reason:
      "Keep the synthetic lifecycle proposal non-authoritative until the platform matrix is green.",
    actor: USER,
  }),
);
...
if (mode === "migration-failure") {
  ...
  sabotage.exec(
    "CREATE TABLE correction_appeals (appeal_id TEXT PRIMARY KEY) STRICT",
  );
  sabotage.close();
} else if (mode === "future") {
  ...
  const now = Date.now();
  future.run(
    "INSERT INTO migrations (version, name, status, runtime_version, started_at, finished_at) VALUES (21, '021-ri53-future-fixture', 'completed', 'future-runtime', ?, ?)",
    now,
    now,
  );
  future.close();
...
const bindingHash = createHash("sha256")
  .update(
    [project.id, decision.id, issue.id, episode.id, receipt.id].join("\0"),
  )
  .digest("hex");
