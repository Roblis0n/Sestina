...
const FORBIDDEN_CONTENT = Object.freeze([
  ...
  /AKIA[0-9A-Z]{16}/u,
  /[A-Za-z]:\\(?:Users|Documents and Settings|Codex work|Vibe)\\/u,
  /\/Users\/[^/\s]+\//u,
]);
...
function validateIdentity(identity) {
  exactKeys(
    ...
    [
      ...
      "runtimeVersion",
      "reportSchemaVersion",
      "capsuleResponseSchemaVersion",
      "mcpServerVersion",
      "mcpResearchContextSchemaVersion",
      "checkerBuildContract",
      "supportedSchemaMinimum",
      "futureSchemaPolicy",
      "downgradeSupported",
      "databaseSchemaVersion",
      "migrationManifestVersion",
      "migrationCount",
  ...
  invariant(
    JSON.stringify(identity) === JSON.stringify(SESTINA_RELEASE_IDENTITY),
    "release_identity_not_canonical",
  );
  ...
  invariant(
    /^[a-f0-9]{64}$/u.test(identity.migrationManifestHash),
    "migration_manifest_hash_invalid",
  );
  invariant(
    /^[a-f0-9]{64}$/u.test(identity.releaseBuildId),
...
export function validateReleaseManifest(manifest) {
  ...
  exactKeys(
    manifest.platform,
    ["os", "architecture", "nativeSecretBackend"],
    "release_platform",
  );
  invariant(
    ["win32", "darwin", "linux"].includes(manifest.platform.os),
    "release_platform_os_invalid",
  );
  invariant(
    (manifest.platform.os === "win32" &&
  ...
  invariant(
    manifest.platform.nativeSecretBackend === backend,
    "release_native_secret_backend_invalid",
  );

  exactKeys(manifest.source, ["gitCommit"], "release_source");
  invariant(
    /^[a-f0-9]{40}$/u.test(manifest.source.gitCommit),
  ...
  const platformSlug =
    platformSlugs[`${manifest.platform.os}-${manifest.platform.architecture}`];
  exactKeys(
  ...
  invariant(
    manifest.compatibility.nodeRange === manifest.identity.nodeRange &&
      manifest.compatibility.supportedSchemaMinimum ===
        manifest.identity.supportedSchemaMinimum &&
      manifest.compatibility.supportedSchemaMaximum ===
        manifest.identity.databaseSchemaVersion &&
      manifest.compatibility.futureSchemaPolicy === "fail_closed" &&
      manifest.compatibility.downgradeSupported === false,
    "release_compatibility_invalid",
  );

  exactKeys(
    manifest.contents,
    ["releaseBundleRoot", "releaseBundlePaths", "executablePaths"],
    "release_contents",
  );
  const expectedRoot = `sestina-research-room-${manifest.identity.version}-${platformSlug}`;
  invariant(
    manifest.contents.releaseBundleRoot === expectedRoot,
    "release_bundle_root_invalid",
  );
  sortedUniqueStrings(
    manifest.contents.releaseBundlePaths,
    "release_bundle_paths",
  );
  sortedUniqueStrings(
    manifest.contents.executablePaths,
    "release_executable_paths",
  );
  invariant(
    manifest.contents.releaseBundlePaths.every((path) =>
      path.startsWith(`${expectedRoot}/`),
    ),
    "release_bundle_path_outside_root",
  );
  invariant(
    JSON.stringify(manifest.contents.executablePaths) ===
      JSON.stringify(
        [
          `${expectedRoot}/app/main.js`,
          `${expectedRoot}/app/mcp/main.js`,
          `${expectedRoot}/start.mjs`,
        ].sort((left, right) => left.localeCompare(right, "en")),
      ),
    "release_executable_paths_invalid",
  );
  ...
  exactKeys(
    ...
    [
      ...
      "postinstall",
      "containsSourceMaps",
      "containsResearchData",
      "containsCredentials",
      "uninstallDeletesProjectData",
      "npmPublished",
    ],
    "release_security",
  );
  ...
  invariant(
    JSON.stringify(manifest.security) === JSON.stringify(expectedSecurity),
    "release_security_contract_invalid",
  );
  ...
  for (const artifact of manifest.artifacts) {
    ...
    invariant(
      safeArtifactName(artifact.file) &&
        expectedArtifacts.get(artifact.file) === artifact.kind,
      "release_artifact_name_or_kind_invalid",
    );
    invariant(
  ...
  const expectedPrimaryArtifact =
    manifest.platform.os === "win32"
      ? `${expectedRoot}.zip`
      : `${expectedRoot}.tar.gz`;
  invariant(
    manifest.distribution.primaryArtifact === expectedPrimaryArtifact,
    "release_primary_artifact_invalid",
  );
}
  ...
  for (const entry of entries) {
    ...
    invariant(
      !segments.some((segment) =>
        FORBIDDEN_SEGMENTS.has(segment.toLocaleLowerCase("en-US")),
      ),
      `forbidden_release_path:${normalized}`,
    );
    invariant(
      !/\.(?:db|sqlite3?|log|pem|key|p12|map)$/iu.test(normalized),
      `forbidden_release_path:${normalized}`,
    );
    invariant(
    ...
    if (TEXT_EXTENSIONS.has(extension(normalized))) {
      ...
      for (const pattern of FORBIDDEN_CONTENT)
        invariant(
          !pattern.test(text),
          `forbidden_release_content:${normalized}`,
        );
    }
  }
}
...
function assertRequiredPath(
  paths,
  path,
  label = "release_bundle_runtime_missing",
) {
...
export function verifyReleaseBundleEntries(entries, manifest) {
  ...
  invariant(
    [...files.keys()].some(
      (path) =>
        path.startsWith(`${root}/app/client/assets/`) && path.endsWith(".js"),
    ),
    "release_client_javascript_missing",
  );
  invariant(
    [...files.keys()].some(
      (path) =>
        path.startsWith(`${root}/app/client/assets/`) && path.endsWith(".css"),
    ),
    "release_client_styles_missing",
  );

  const packageJson = JSON.parse(
    files.get(`${root}/package.json`).data.toString("utf8"),
  );
  exactKeys(
    packageJson,
    [
  ...
  invariant(
    packageJson.name === manifest.identity.package &&
      packageJson.version === manifest.identity.version &&
      packageJson.private === true &&
      packageJson.license === "Apache-2.0" &&
      packageJson.type === "module" &&
      JSON.stringify(packageJson.engines) ===
        JSON.stringify({ node: manifest.identity.nodeRange }) &&
      JSON.stringify(packageJson.scripts) ===
        JSON.stringify({ start: "node start.mjs" }),
    "release_package_identity_invalid",
  );
  const releaseIdentity = JSON.parse(
    files.get(`${root}/RELEASE-IDENTITY.json`).data.toString("utf8"),
  );
  invariant(
  ...
  for (const path of manifest.contents.executablePaths)
    invariant(
      files.get(path)?.mode === 0o755,
      `release_executable_mode_invalid:${path}`,
    );
  if (manifest.platform.os === "win32") {
    assertRequiredPath(
      files,
      `${root}/node_modules/@primno/dpapi/package.json`,
      "release_dpapi_runtime_missing",
    );
    assertRequiredPath(
      files,
      `${root}/node_modules/@primno/dpapi/prebuilds/win32-${manifest.platform.architecture}/@primno+dpapi.node`,
      "release_dpapi_runtime_missing",
    );
    assertRequiredPath(
      files,
    ...
    assertRequiredPath(
      files,
      `${root}/node_modules/@napi-rs/keyring/package.json`,
      "release_keyring_runtime_missing",
    );
    invariant(
      [...files.keys()].some(
        (path) =>
          path.startsWith(`${root}/node_modules/@napi-rs/${nativePackage}/`) &&
          path.endsWith(".node"),
      ),
      "release_keyring_native_binary_missing",
    );
  }
}
...
export function parseChecksums(content) {
  ...
  for (const line of lines) {
    ...
    invariant(
      match !== null && !result.has(match[2]),
      "sha256sums_format_invalid",
    );
  ...
  for (let index = 0; index < left.length; index += 1) {
    invariant(
      left[index].path === right[index].path &&
        ...
        left[index].data.equals(right[index].data),
      `release_archive_content_drift:${left[index]?.path ?? index}`,
    );
  }
}
...
export async function verifyReleaseDirectory(releaseDirectory) {
  ...
  invariant(
    ...
    "release_directory_files_invalid",
  );
  const checksums = parseChecksums(
    await readFile(join(releaseDirectory, "SHA256SUMS"), "utf8"),
  );
  invariant(
    checksums.size === manifest.artifacts.length + 1,
    "sha256sums_file_count_invalid",
  );
  invariant(
    checksums.get("release-manifest.json") === sha256(manifestBytes),
  ...
  for (const artifact of manifest.artifacts) {
    ...
    invariant(
      (await stat(path)).isFile() && bytes.length === artifact.size,
      `release_artifact_size_mismatch:${artifact.file}`,
    );
    invariant(
      sha256(bytes) === artifact.sha256 &&
        checksums.get(artifact.file) === artifact.sha256,
      `release_artifact_checksum_mismatch:${artifact.file}`,
    );
  }
  const tar = manifest.artifacts.find(
    (artifact) => artifact.kind === "platform-tar-gzip",
  );
  const zip = manifest.artifacts.find(
  ...
  return Object.freeze({
    manifest,
    verifiedFiles: Object.freeze(expectedTopLevel),
  });
}
