...
export function validateArchiveEntries(entries, limits = DEFAULT_LIMITS) {
  if (
    ...
    throw new Error("archive_file_count_invalid");
  const exact = new Set();
  const folded = new Set();
  let total = 0;
  const validated = entries.map((entry) => {
    if (unsafePath(entry?.path))
      throw new Error(`unsafe_archive_path:${entry?.path ?? ""}`);
    const path = entry.path.normalize("NFC");
    const collision = path.toLocaleLowerCase("en-US");
    if (exact.has(path) || folded.has(collision))
      throw new Error(`archive_path_collision:${path}`);
    exact.add(path);
    ...
    if (data.length > limits.maxEntryBytes)
      throw new Error(`archive_entry_too_large:${path}`);
    total += data.length;
    if (total > limits.maxTotalBytes)
      throw new Error("archive_total_too_large");
    const mode = entry.mode ?? 0o644;
    if (mode !== 0o644 && mode !== 0o755)
      throw new Error(`unsafe_archive_mode:${path}`);
...
function createTar(entries, limits) {
  const blocks = [];
  for (const entry of validateArchiveEntries(entries, limits)) {
...
export async function createDeterministicTarGzip(outputPath, entries, limits) {
  const compressed = gzipSync(createTar(entries, limits), {
    level: 9,
    mtime: 0,
  });
...
function inspectTarBuffer(buffer, limits = DEFAULT_LIMITS) {
  ...
  while (offset + 512 <= buffer.length) {
    ...
    if (checksumHeader.reduce((sum, byte) => sum + byte, 0) !== storedChecksum)
      throw new Error("invalid_tar_checksum");
    const type = header[156];
    if (type !== 0 && type !== 0x30)
      throw new Error("unsafe_archive_entry_type");
    const path = tarName(header);
    const size = parseOctal(header, 124, 12);
    const mode = parseOctal(header, 100, 8) & 0o777;
  ...
  if (buffer.subarray(offset).some((byte) => byte !== 0))
    throw new Error("invalid_tar_trailing_data");
...
export async function inspectTarGzip(inputPath, limits = DEFAULT_LIMITS) {
  ...
  try {
    expanded = gunzipSync(compressed, {
      maxOutputLength: limits.maxTotalBytes + limits.maxFiles * 1024,
    });
  } catch {
    throw new Error("invalid_or_oversized_tar_gzip");
  }
  if (expanded.length > Math.max(compressed.length * 200, 1024 * 1024))
...
export async function createDeterministicZip(outputPath, entries) {
  ...
  await mkdir(dirname(outputPath), { recursive: true });
  await writeFile(outputPath, Buffer.concat([...locals, centralBuffer, end]));
}
...
function inspectZipBuffer(buffer, limits = DEFAULT_LIMITS) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (
      ...
      throw new Error("unsafe_zip_features");
    if (
    ...
    if (
      ...
      buffer.readUInt32LE(localOffset + 14) !==
...
async function extractEntries(entries, outputDirectory) {
  ...
  for (const entry of entries) {
    ...
    if (target !== root && !target.startsWith(`${root}${sep}`))
      throw new Error("archive_extraction_escape");
...
export async function extractTarGzip(
  ...
  await extractEntries(
    await inspectTarGzip(inputPath, limits),
    outputDirectory,
  );
}
