export type ProjectWorkspace = "review" | "overview" | "brief" | "decision" | "issue" | "evidence" | "episode" | "receipt" | "appeal" | "attention" | "not_found";
export interface ProjectRoute { readonly workspace: ProjectWorkspace; readonly objectId?: string; readonly creating?: boolean; }

const COLLECTIONS = Object.freeze({ decisions: "decision", issues: "issue", evidence: "evidence", episodes: "episode", receipts: "receipt", appeals: "appeal" } as const);
const PREFIXES = Object.freeze({ decision: "rdec_", issue: "riss_", evidence: "revd_", episode: "repi_", receipt: "rrcp_", appeal: "rapl_" } as const);
...
export function parseProjectRoute(pathname: string): ProjectRoute {
  ...
  const match = /^\/project\/(decisions|issues|evidence|episodes|receipts|appeals)(?:\/([^/]+))?$/u.exec(pathname);
  ...
  if (workspace === "appeal" && objectId === "new") return { workspace, creating: true };
...
export function hrefForRoute(route: ProjectRoute): string {
  ...
  const collection = route.workspace === "decision" ? "decisions" : route.workspace === "issue" ? "issues" : route.workspace === "evidence" ? "evidence" : route.workspace === "episode" ? "episodes" : route.workspace === "receipt" ? "receipts" : "appeals";
