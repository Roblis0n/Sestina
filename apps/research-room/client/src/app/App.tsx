...
export function App() {
  ...
  async function rollbackReceipt(receipt: ResearchRoomReceiptDto, reason: string) {
    await runBusy(async () => { const result = await researchRoomApi.rollbackReceipt(receipt.id, receipt.version, reason); await refreshState(); setInspectorSelection({ kind: "receipt", value: result }); setInspectorOpen(true); setRuntime("ready"); showNotice(t(language, "rolled_back"), "ready"); });
  }
