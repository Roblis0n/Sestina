...
function AppealDetail(props: Props & { readonly appealId: string }) {
  ...
  return <section className="object-workspace appeal-workspace" aria-labelledby="appeal-detail-title">
      <WorkspaceHeader id="appeal-detail-title" eyebrow={`CORRECTION APPEAL · v${detail.version}`} className="appeal-hero" title={detail.disagreement} description={`${detail.criterionId} · ${detail.findingId}`} status={<StatusBadge tone={statusTone(detail.status)}>{detail.status}</StatusBadge>} actions={<><Button type="button" variant="secondary" disabled={loading || busy} onClick={() => { props.onNavigate(`/project/memory?pinKind=appeal&pinId=${encodeURIComponent(detail.id)}`); }}>{c(props.language, "Pin to Project Memory", "固定到项目记忆")}</Button><Button type="button" variant="quiet" disabled={loading || busy} onClick={() => { void load(); }}>{c(props.language, "Reload", "重新加载")}</Button></>} />
    ...
    {detail.status !== "resolved" && latestStatement ? <AppealDeliberationEntry language={props.language} detail={detail} question={latestStatement.secondOpinionQuestion} onNavigate={props.onNavigate} /> : null}
...
function AppealDeliberationEntry({ language, detail, question, onNavigate }: { readonly language: AppLanguage; readonly detail: AppealDetailDto; readonly question: string; readonly onNavigate: (href: string) => void }) {
  const params = new URLSearchParams({ sourceKind: "correction_appeal", source: detail.id, question, title: c(language, `Unresolved appeal: ${detail.disagreement}`, `未解决申诉：${detail.disagreement}`) });
  return <section className="deliberation-entry" aria-labelledby="appeal-deliberation-entry-title"><div><p className="eyebrow">UNRESOLVED AFTER APPEAL</p><h2 id="appeal-deliberation-entry-title">{c(language, "Escalate this unresolved conflict to two bounded views", "将这个未解决冲突升级为双视角有限会商")}</h2><p>{c(language, "The Appeal and original Finding remain immutable. A Room Resolution cannot bypass their existing Authority Gate.", "Appeal 与原 Finding 保持不变；Room Resolution 不能绕过它们现有的 Authority Gate。")}</p></div><Button type="button" variant="secondary" onClick={() => { onNavigate(`/project/deliberation-rooms/new?${params.toString()}`); }}>{c(language, "Open Deliberation Room", "打开会商室")}</Button></section>;
}
