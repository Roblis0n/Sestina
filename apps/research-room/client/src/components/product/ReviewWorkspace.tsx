...
export function ReviewWorkspace(props: ReviewWorkspaceProps) {
  ...
  const reviewStatus = analyzed?.providerStatus ?? (prepared ? t(props.language, "manifest_unsent") : t(props.language, "proposal_only"));
  return <section className="review-workspace" aria-labelledby="review-heading">
    ...
    />
    {prepared ? <section className="review-current-action" aria-label={props.language === "en" ? "Next safe action" : "下一安全行动"}>
      <div className="review-current-action__object">
        <p className="eyebrow">{props.language === "en" ? "CURRENT OBJECT" : "当前对象"}</p>
        <h2>{analyzed ? t(props.language, "analysis_title") : t(props.language, "manifest_title")}</h2>
        <p>{analyzed
          ? props.language === "en" ? "The analysis is a proposal. Only your recorded disposition can change project state." : "分析结果仍是提案；只有你记录的处置才能改变项目状态。"
          : props.language === "en" ? "The exact Context Manifest is prepared and remains unsent until you explicitly continue." : "精确 Context Manifest 已生成；在你明确继续前，它仍未发送。"}</p>
      </div>
      <dl>
        <div><dt>{props.language === "en" ? "Status" : "状态"}</dt><dd><StatusBadge tone={analyzed?.providerStatus === "semantic_ready" ? "ready" : "warning"}>{reviewStatus}</StatusBadge></dd></div>
        <div><dt>{props.language === "en" ? "Authority" : "权威"}</dt><dd>user_only</dd></div>
        <div><dt>{props.language === "en" ? "Source" : "来源"}</dt><dd>{props.providerMode === "configured" ? "Manifest + Provider" : "local ledger"}</dd></div>
      </dl>
      <div className="review-current-action__next">
        <p className="eyebrow">{props.language === "en" ? "NEXT SAFE ACTION" : "下一安全行动"}</p>
        {!analyzed
          ? <Button data-inspector-return="manifest" type="button" variant="primary" disabled={props.busy} onClick={() => void analyze()}>{t(props.language, "confirm_analyze")}</Button>
          : <Button data-inspector-return="analysis" type="button" variant="primary" onClick={() => { document.getElementById("reason")?.focus(); }}>{props.language === "en" ? "Review and record disposition" : "核对并记录处置"}</Button>}
      </div>
    </section> : null}
    <div className="thread">
      ...
      {prepared ? <article className="thread-event thread-event--proposal thread-event--complete"><span className="thread-event__index" aria-label={props.language === "en" ? "Completed step 2" : "已完成步骤 2"}>✓</span><div className="thread-event__body"><div className="event-title"><h2>{t(props.language, "suggestion")}</h2><StatusBadge tone="ready">{props.language === "en" ? "Bound to Manifest" : "已绑定 Manifest"}</StatusBadge></div><p className="review-proposal-text">{suggestion}</p><dl className="review-proposal-facts"><div><dt>{t(props.language, "evidence_class")}</dt><dd>{evidenceClass}</dd></div><div><dt>{props.language === "en" ? "Working Memory" : "工作记忆"}</dt><dd>{selectedMemoryItemIds.length}</dd></div></dl></div></article> : <article className="thread-event thread-event--proposal"><span className="thread-event__index">02</span><div className="thread-event__body"><h2>{t(props.language, "suggestion")}</h2>
