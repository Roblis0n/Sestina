...
export function ProjectShell(props: ProjectShellProps) {
  ...
  return <main id="main-content" className="project-shell" data-navigation-collapsed={collapsed} data-inspector-open={props.inspectorOpen}>
    ...
    <div id="workspace-heading" className="project-workspace" tabIndex={-1}>
      ...
      {route.workspace === "overview" || route.workspace === "review" ? <aside className="pilot-entry" aria-label={props.language === "en" ? "Closed External App Pilot" : "封闭式外部 App Pilot"}><div><p className="eyebrow">CODEX · READ ONLY · PROPOSAL ONLY</p><strong>{props.language === "en" ? "Bring one bounded Codex proposal through Sestina Review" : "把一条有边界的 Codex 建议带入 Sestina Review"}</strong><span>{props.language === "en" ? "Preview exact outbound context first. Import never means acceptance." : "先核对精确外发内容；导入永远不等于接受。"}</span></div><Button type="button" variant="secondary" onClick={() => { navigate("/project/external-app-pilot"); }}>{props.language === "en" ? "Open Pilot" : "打开 Pilot"}</Button></aside> : null}
