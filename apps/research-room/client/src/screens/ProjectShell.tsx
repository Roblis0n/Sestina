  ...
  return <main id="main-content" className="project-shell" data-navigation-collapsed={collapsed} data-inspector-open={props.inspectorOpen}>
    ...
    <div id="workspace-heading" className="project-workspace" tabIndex={-1}>
      <form className="project-search" role="search" aria-label={props.language === "en" ? "Search this project" : "搜索当前项目"} onSubmit={(event) => { void runSearch(event); }}>
        ...
        <div><input id="project-search-input" value={searchQuery} onChange={(event) => { setSearchQuery(event.target.value); if (!event.target.value) clearSearch(); }} placeholder={props.language === "en" ? "Brief, Decision, Issue, Evidence, Episode, Receipt, Appeal, Deliberation" : "Brief、Decision、Issue、Evidence、Episode、Receipt、Appeal、会商室"} /><Button type="submit" variant="quiet" disabled={searching}>{searching ? "…" : props.language === "en" ? "Search" : "搜索"}</Button>{search ? <Button type="button" variant="quiet" onClick={clearSearch}>{props.language === "en" ? "Clear" : "清除"}</Button> : null}</div>
