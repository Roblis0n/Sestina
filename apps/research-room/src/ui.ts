...
.provider-toggle { border: 1px solid rgb(255 255 255 / 34%); border-radius: 2px; padding: 6px 10px; color: #eef3ef; background: transparent; font-size: .7rem; font-weight: 700; }
.provider-toggle:hover { background: rgb(255 255 255 / 9%); }
...
.manifest-details { margin-top: .75rem; border: 1px solid #d4d5ce; background: #f1f0ea; }
.manifest-details summary { cursor: pointer; padding: .62rem .7rem; color: #33443d; font-size: .7rem; font-weight: 700; }
.manifest-details pre { max-height: 250px; margin: 0; padding: .75rem; overflow: auto; border-top: 1px solid #d4d5ce; background: #fbfbf8; color: #354039; font: .64rem/1.5 Consolas, monospace; white-space: pre-wrap; overflow-wrap: anywhere; }
.assessment { margin: .55rem 0; padding: .68rem .72rem; border: 1px solid #d4d7d1; border-left: 3px solid #7f958b; background: #f4f4ef; }
.assessment[data-verdict="positive"] { border-left-color: var(--danger); }
.assessment[data-verdict="unknown"] { border-left-color: var(--amber); }
.assessment-head { display: flex; justify-content: space-between; gap: 10px; font: 700 .7rem Consolas, monospace; }
.assessment p { margin: .45rem 0 0; font-size: .73rem; }
.assessment blockquote { margin: .48rem 0 0; padding-left: .65rem; border-left: 1px solid #aeb6b0; color: #59635e; font-size: .7rem; }
.authority-gate { margin-top: 1rem; padding: .8rem; border: 1px solid #8c9b94; background: #ebefeb; }
.authority-gate h3 { margin-top: 0; padding-top: 0; border-top: 0; }
.provider-dialog { width: min(720px, calc(100vw - 48px)); max-height: calc(100vh - 48px); padding: 0; border: 1px solid #5c6c64; border-radius: 2px; background: var(--paper); color: var(--ink); box-shadow: 0 24px 80px rgb(20 31 26 / 34%); }
.provider-dialog::backdrop { background: rgb(22 34 29 / 52%); backdrop-filter: blur(2px); }
.provider-dialog header { padding: 22px 26px 18px; border-bottom: 1px solid var(--line); }
.provider-dialog h2 { margin: 0; font-family: Georgia, serif; font-size: 1.45rem; font-weight: 500; }
.provider-dialog header p { margin: 7px 0 0; color: var(--muted); font-size: .76rem; line-height: 1.55; }
.provider-form { padding: 12px 26px 24px; }
.provider-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0 14px; }
.provider-grid .wide { grid-column: 1 / -1; }
.provider-status-box { margin-top: 14px; padding: 10px 12px; border-left: 2px solid var(--forest); background: #edf0eb; }
.provider-note { color: var(--muted); font-size: .69rem; line-height: 1.55; }
...
<body>
  <header class="topbar">
    ...
    <div class="topbar-tools">
      ...
      <button id="provider-settings-button" class="provider-toggle" type="button" data-i18n="provider_settings" hidden>Provider 设置</button>
  ...
  <dialog id="provider-dialog" class="provider-dialog">
    <header><h2 data-i18n="provider_heading">Semantic Judge Provider</h2><p data-i18n="provider_deck">只配置一个 OpenAI-compatible 端点。保存配置不会测试网络，也不会调用模型。</p></header>
    <form id="provider-form" class="provider-form">
      <div class="provider-grid">
        <div><label for="provider-id" data-i18n="provider_id_label">Provider 名称</label><input id="provider-id" required maxlength="128" autocomplete="off" value="openai-compatible"></div>
        <div><label for="provider-model" data-i18n="provider_model_label">模型</label><input id="provider-model" required maxlength="256" autocomplete="off" placeholder="judge-model"></div>
        <div class="wide"><label for="provider-url" data-i18n="provider_url_label">Base URL</label><input id="provider-url" required maxlength="2048" autocomplete="url" placeholder="http://127.0.0.1:11434/v1"></div>
        <div><label for="provider-timeout" data-i18n="provider_timeout_label">超时（毫秒）</label><input id="provider-timeout" type="number" min="100" max="120000" value="15000" required></div>
        <div><label for="provider-max-tokens" data-i18n="provider_tokens_label">最大输出 tokens（可选）</label><input id="provider-max-tokens" type="number" min="1" max="65536"></div>
        <div class="wide"><label for="provider-key" data-i18n="provider_key_label">API key（外部 HTTPS 必填）</label><input id="provider-key" type="password" maxlength="8192" autocomplete="new-password" data-i18n-placeholder="provider_key_placeholder" placeholder="保存在操作系统安全存储中"></div>
      </div>
      <p class="provider-note" data-i18n="provider_security_note">API key 不进入项目、配置文件、收据、日志或页面状态。外部地址只允许 HTTPS；HTTP 只允许明确的本机回环地址。</p>
      <div id="provider-status-box" class="provider-status-box status"></div>
      <div class="actions"><button id="save-provider" class="primary-action" type="submit" data-i18n="provider_save">保存配置</button><button id="delete-provider-config" class="secondary" type="button" data-i18n="provider_delete_config">删除配置</button><button id="delete-provider-key" class="danger" type="button" data-i18n="provider_delete_key">删除 API key</button><button id="close-provider" class="secondary" type="button" data-i18n="close_button">关闭</button></div>
    </form>
  </dialog>
  <main>
    ...
    <section id="room" class="room-shell" hidden>
      ...
      <div class="grid">
        ...
        <article class="panel review-panel"><p class="panel-kicker">02 / REVIEW</p><h2 data-i18n="review_heading">审议一个建议</h2><p class="muted" data-i18n="review_deck">一次只审议一项建议。先核对发送清单，再决定是否让 Provider 分析。</p>
          ...
          <section id="manifest" class="manifest" hidden><h3 data-i18n="manifest_heading">Context Manifest（发送前）</h3><p id="manifest-summary" class="status"></p><p id="manifest-protocol" class="status"></p><table><thead><tr><th data-i18n="field_col">字段</th><th data-i18n="source_col">来源</th><th data-i18n="sensitivity_col">敏感性</th></tr></thead><tbody id="manifest-fields"></tbody></table><p><strong data-i18n="excluded_prefix">明确排除：</strong><span id="manifest-excluded"></span></p><details id="manifest-request" class="manifest-details" hidden><summary data-i18n="exact_request_heading">查看将要发送的精确请求体</summary><pre id="manifest-request-body"></pre></details><div class="actions"><button id="analyze" class="primary-action" type="button" data-i18n="analyze_button">我已核对，开始分析</button><button id="cancel-review" class="secondary" type="button" data-i18n="cancel_button">取消</button></div></section>
          <section id="analysis" class="manifest" hidden><h3 data-i18n="analysis_heading">分析结果</h3><p id="provider-status" class="status"></p><p id="reasonable-increment" class="status"></p><div id="assessments"></div><h3 data-i18n="derived_findings_heading">Kernel 派生 Findings</h3><div id="findings"></div><p><strong data-i18n="delta_prefix">真正增加：</strong><span id="delta"></span></p><p><strong data-i18n="alternatives_prefix">替代解释：</strong><span id="alternatives"></span></p><p><strong data-i18n="unknowns_prefix">未知项：</strong><span id="unknowns"></span></p><p><strong data-i18n="unproven_prefix">未证明：</strong><span id="unproven"></span></p><p><strong data-i18n="correction_prefix">最小纠正：</strong><span id="correction"></span></p>
            <div class="authority-gate"><h3 data-i18n="authority_heading">Authority Gate — 只有你能处置</h3><p class="muted" data-i18n="authority_deck">模型只提交评估；Kernel 只派生结构化结果。接受、拒绝、修改后接受、暂缓和正式改向都必须由你明确执行。</p>
...
const COPY = {
  "zh-CN": {
    provider_settings: "Provider 设置", provider_heading: "Semantic Judge Provider", provider_deck: "只配置一个 OpenAI-compatible 端点。保存配置不会测试网络，也不会调用模型。", provider_id_label: "Provider 名称", provider_model_label: "模型", provider_url_label: "Base URL", provider_timeout_label: "超时（毫秒）", provider_tokens_label: "最大输出 tokens（可选）", provider_key_label: "API key（外部 HTTPS 必填）", provider_key_placeholder: "保存在操作系统安全存储中", provider_security_note: "API key 不进入项目、配置文件、收据、日志或页面状态。外部地址只允许 HTTPS；HTTP 只允许明确的本机回环地址。", provider_save: "保存配置", provider_delete_config: "删除配置", provider_delete_key: "删除 API key", close_button: "关闭", provider_offline: "未配置 Provider；默认 ledger_only", provider_configured: "已配置", provider_secret_yes: "安全密钥：已配置", provider_secret_no: "安全密钥：未配置", provider_saved: "Provider 配置已保存；没有发起网络请求。", provider_reopen: "当前项目需重新打开，新的配置才会生效。", provider_config_deleted: "Provider 配置已删除；安全密钥保持不变。", provider_key_deleted: "API key 已从操作系统安全存储删除。", excluded_prefix: "明确排除：", exact_request_heading: "查看将要发送的精确请求体", derived_findings_heading: "Kernel 派生 Findings", unknowns_prefix: "未知项：", authority_heading: "Authority Gate — 只有你能处置", authority_deck: "模型只提交评估；Kernel 只派生结构化结果。接受、拒绝、修改后接受、暂缓和正式改向都必须由你明确执行。", assessment_unknown: "信息不足", model_proposed: "模型提出", system_derived: "系统派生", reasonable_prefix: "合理增量",
  ...
  en: {
    provider_settings: "Provider settings", provider_heading: "Semantic Judge Provider", provider_deck: "Configure one OpenAI-compatible endpoint. Saving does not test the network or call a model.", provider_id_label: "Provider name", provider_model_label: "Model", provider_url_label: "Base URL", provider_timeout_label: "Timeout (milliseconds)", provider_tokens_label: "Maximum output tokens (optional)", provider_key_label: "API key (required for external HTTPS)", provider_key_placeholder: "Stored in operating-system secure storage", provider_security_note: "The API key never enters projects, config files, receipts, logs, or page state. External targets require HTTPS; HTTP is allowed only for explicit loopback targets.", provider_save: "Save configuration", provider_delete_config: "Delete configuration", provider_delete_key: "Delete API key", close_button: "Close", provider_offline: "No Provider configured; ledger_only is the default", provider_configured: "Configured", provider_secret_yes: "Secure key: configured", provider_secret_no: "Secure key: not configured", provider_saved: "Provider configuration saved; no network request was made.", provider_reopen: "Reopen the current project before the new configuration takes effect.", provider_config_deleted: "Provider configuration deleted; the secure key was retained.", provider_key_deleted: "The API key was deleted from operating-system secure storage.", excluded_prefix: "Explicitly excluded: ", exact_request_heading: "Inspect the exact request body that will be sent", derived_findings_heading: "Kernel-derived Findings", unknowns_prefix: "Unknowns: ", authority_heading: "Authority Gate — only you can dispose", authority_deck: "The model submits assessments; the Kernel derives structured results. Accept, reject, accept after modification, defer, and formal redirect all require your explicit action.", assessment_unknown: "insufficient context", model_proposed: "model-proposed", system_derived: "system-derived", reasonable_prefix: "Reasonable increment",
...
const PROVIDER_ERROR_COPY = {
  "zh-CN": { invalid_provider_config: "Provider 配置无效。", provider_config_corrupt: "Provider 配置已损坏；原文件已保留。", provider_key_required: "外部 HTTPS Provider 必须配置安全 API key。", secure_storage_unavailable: "操作系统安全存储不可用。", provider_settings_unavailable: "Provider 设置不可用。", provider_configuration_changed: "Provider 配置已改变，请重新打开项目并重新生成 Manifest。" },
  en: { invalid_provider_config: "The Provider configuration is invalid.", provider_config_corrupt: "The Provider configuration is corrupt; the original file was preserved.", provider_key_required: "An external HTTPS Provider requires a securely stored API key.", secure_storage_unavailable: "Operating-system secure storage is unavailable.", provider_settings_unavailable: "Provider settings are unavailable.", provider_configuration_changed: "The Provider configuration changed. Reopen the project and prepare a new Manifest." }
};
const ERROR_COPY = {
...
async function api(path, options = {}) {
  ...
  if (!response.ok || value.ok !== true) { const code = value.error?.code || "request_failed"; const fallback = currentLanguage === "en" ? value.error?.message : undefined; const error = new Error(PROVIDER_ERROR_COPY[currentLanguage][code] || ERROR_COPY[currentLanguage][code] || fallback || t("request_failed")); error.code = code; throw error; }
  return value.value;
}
...
async function saveLanguage(language, button, firstRun) {
  ...
  try { await whileBusy(button, firstRun ? "Saving locally / 正在本机保存……" : t("language_saved"), () => api("/api/preferences/language", { method: "POST", mutation: true, body: { language } })); applyLanguage(language); $("language-setup").hidden = true; $("language-controls").hidden = false; $("provider-settings-button").hidden = false; if ($("project-setup").hidden && $("room").hidden) $("project-launch").hidden = false; await loadProviderStatus(); live(t("service_ready"), "success"); }
...
function showProviderStatus(status) {
  const box = $("provider-status-box");
  if (status.mode !== "configured" || !status.config) { box.textContent = t("provider_offline") + "\n" + (status.secretConfigured ? t("provider_secret_yes") : t("provider_secret_no")); return; }
  const config = status.config;
  box.textContent = t("provider_configured") + " · " + config.family + " · " + config.locality + "\n" + config.providerId + " / " + config.model + "\n" + config.baseUrl + "\n" + (status.secretConfigured ? t("provider_secret_yes") : t("provider_secret_no")) + " · generation " + config.generation;
  $("provider-id").value = config.providerId; $("provider-model").value = config.model; $("provider-url").value = config.baseUrl; $("provider-timeout").value = String(config.timeoutMs); $("provider-max-tokens").value = config.maxOutputTokens ? String(config.maxOutputTokens) : "";
}
async function loadProviderStatus() { try { const status = await api("/api/provider"); showProviderStatus(status); return status; } catch (error) { $("provider-status-box").textContent = error.message; return undefined; } }
$("provider-settings-button").onclick = async () => { await loadProviderStatus(); $("provider-dialog").showModal(); };
$("close-provider").onclick = () => $("provider-dialog").close();
$("provider-form").addEventListener("submit", async (event) => { event.preventDefault(); const maxTokens = $("provider-max-tokens").value.trim(); const key = $("provider-key").value; try { const status = await whileBusy($("save-provider"), t("provider_saved"), () => api("/api/provider", { method: "POST", mutation: true, body: { providerId: $("provider-id").value, baseUrl: $("provider-url").value, model: $("provider-model").value, timeoutMs: Number($("provider-timeout").value), ...(maxTokens ? { maxOutputTokens: Number(maxTokens) } : {}), ...(key ? { apiKey: key } : {}) } })); $("provider-key").value = ""; showProviderStatus(status); live(t("provider_saved") + (status.projectReopenRequired ? " " + t("provider_reopen") : ""), "success"); } catch (error) { live(error.message, "error"); } });
$("delete-provider-config").onclick = async () => { try { const status = await api("/api/provider/config", { method: "DELETE", mutation: true }); $("provider-form").reset(); $("provider-id").value = "openai-compatible"; $("provider-timeout").value = "15000"; showProviderStatus(status); live(t("provider_config_deleted") + (status.projectReopenRequired ? " " + t("provider_reopen") : ""), "success"); } catch (error) { live(error.message, "error"); } };
$("delete-provider-key").onclick = async () => { try { const status = await api("/api/provider/secret", { method: "DELETE", mutation: true }); $("provider-key").value = ""; showProviderStatus(status); live(t("provider_key_deleted") + (status.projectReopenRequired ? " " + t("provider_reopen") : ""), "success"); } catch (error) { live(error.message, "error"); } };
...
$("prepare-form").addEventListener("submit", async (event) => { event.preventDefault(); try { prepared = await api("/api/reviews/prepare", { method: "POST", mutation: true, body: { suggestion: $("suggestion").value, evidenceClass: $("evidence-class").value } }); analyzed = undefined; $("analysis").hidden = true; $("manifest").hidden = false; const m = prepared.manifest; const judge = m.semanticJudge; $("manifest-summary").textContent = judge ? t("manifest_provider") + ": " + judge.provider.id + " / " + judge.provider.model + " · " + judge.provider.locality + " · " + t("manifest_network") + ": " + m.networkRequired + "\n" + judge.request.endpoint + " · " + judge.request.requestBodyBytes + " bytes · " + t("manifest_unsent") + " · " + t("external_false") : t("manifest_provider") + ": none · ledger_only · " + t("manifest_unsent") + " · " + t("external_false"); $("manifest-protocol").textContent = judge ? "protocol " + judge.protocol.version + " / " + judge.protocol.hash + "\nprompt " + judge.prompt.version + " / " + judge.prompt.hash + "\nrubric " + judge.rubric.version + " / " + judge.rubric.hash + "\nrequest / " + judge.request.requestHash + "\nbody / " + judge.request.requestBodyHash : ""; $("manifest-excluded").textContent = judge ? judge.excludedFields.join(" · ") : t("none"); $("manifest-request").hidden = !judge; $("manifest-request-body").textContent = judge ? judge.request.requestBody : ""; const body = $("manifest-fields"); body.replaceChildren(); for (const field of m.fields) { const row = document.createElement("tr"); for (const value of [field.category, field.source, field.sensitivity]) { const td = document.createElement("td"); td.textContent = value; row.append(td); } body.append(row); } live(t("manifest_ready"), "success"); } catch (error) { live(error.message, "error"); } });
$("cancel-review").onclick = async () => { if (!prepared) return; try { await api("/api/reviews/cancel", { method: "POST", mutation: true, body: { reviewId: prepared.reviewId, confirmationNonce: prepared.confirmationNonce, manifestHash: prepared.manifestHash } }); prepared = undefined; analyzed = undefined; $("manifest").hidden = true; $("analysis").hidden = true; live(t("review_cancelled"), "success"); } catch (error) { live(error.message, "error"); } };
$("analyze").onclick = async () => { if (!prepared) return; try { analyzed = await api("/api/reviews/analyze", { method: "POST", mutation: true, body: { reviewId: prepared.reviewId, confirmationNonce: prepared.confirmationNonce, manifestHash: prepared.manifestHash } }); $("analysis").hidden = false; $("provider-status").textContent = analyzed.providerStatus + (analyzed.ledgerOnlyReason ? " · " + analyzed.ledgerOnlyReason : "") + "\n" + (analyzed.manifest.networkUsed ? "network used" : "network not used"); const trace = analyzed.semanticJudge; $("reasonable-increment").textContent = trace ? t("reasonable_prefix") + ": " + trace.reasonableIncrement.status + " · " + t("system_derived") + " · blocking: " + (trace.reasonableIncrement.blockingCriteria.join(", ") || t("none")) : t("reasonable_prefix") + ": unproven · ledger_only"; const assessments = $("assessments"); assessments.replaceChildren(); if (trace) for (const assessment of trace.assessments) { const card = document.createElement("section"); card.className = "assessment"; card.dataset.verdict = assessment.verdict; const head = document.createElement("div"); head.className = "assessment-head"; const criterion = document.createElement("span"); criterion.textContent = assessment.criterionId; const verdict = document.createElement("span"); verdict.textContent = assessment.verdict + " · " + t("model_proposed"); head.append(criterion, verdict); card.append(head); const rationale = document.createElement("p"); rationale.textContent = assessment.publicRationale; card.append(rationale); if (assessment.verdict === "unknown") { const missing = document.createElement("p"); missing.textContent = t("assessment_unknown") + ": " + assessment.missingContext.join(" · "); card.append(missing); } for (const span of assessment.evidenceSpans) { const quote = document.createElement("blockquote"); quote.textContent = span.quote + " [" + span.start + "–" + span.end + "] " + span.quoteHash; card.append(quote); } assessments.append(card); } const findings = $("findings"); findings.replaceChildren(); for (const finding of analyzed.analysis.findings) { const div = document.createElement("div"); div.className = "finding"; const traceFinding = trace?.findings.find((item) => item.kind === ({ target_substitution: "focus_substitution", repeated_audit: "repeated_audit", argument_leap: "argument_leap", pseudo_depth: "shallow_abstraction", evidence_gap: "evidence_boundary" }[finding.kind] || finding.kind)); div.textContent = finding.kind + " — " + finding.summary + (finding.kind === "reasonable_increment" ? " · " + t("system_derived") : traceFinding ? " · " + t("model_proposed") : ""); findings.append(div); } $("delta").textContent = analyzed.analysis.argumentDelta.genuineAdditions.join("；") || analyzed.analysis.argumentDelta.summary; $("alternatives").textContent = analyzed.analysis.alternativeExplanations.join("；") || t("none"); $("unknowns").textContent = analyzed.analysis.unknowns.join("；") || t("none"); $("unproven").textContent = analyzed.analysis.unproven.join("；"); $("correction").textContent = analyzed.analysis.minimalCorrection; for (const button of $("dispositions").querySelectorAll("button")) button.disabled = analyzed.providerStatus === "ledger_only" && !["rejected", "deferred"].includes(button.dataset.disposition); live(t("analysis_ready"), "success"); } catch (error) { live(error.message, "error"); } };
...
(async () => { try { const status = await api("/api/status"); token = status.sessionToken; if (status.languagePreference === null) { $("language-setup").hidden = false; $("project-launch").hidden = true; $("language-controls").hidden = true; $("provider-settings-button").hidden = true; $("live").textContent = ""; return; } applyLanguage(status.languagePreference); $("language-setup").hidden = true; $("language-controls").hidden = false; $("provider-settings-button").hidden = false; $("project-launch").hidden = false; await loadProviderStatus(); if (!status.directoryPickerAvailable) { $("choose-folder").disabled = true; $("manual-mode").open = true; live(t("picker_unavailable"), "error"); } else live(t("service_ready"), "success"); } catch { live("本地服务不可用 / The local service is unavailable.", "error"); for (const button of document.querySelectorAll("[data-language-choice]")) button.disabled = true; } })();
`;
