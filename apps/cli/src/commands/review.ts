...
function findingView(value: ReviewFinding, verbose: boolean): Readonly<Record<string, unknown>> { return { id: value.id, kind: value.kind, severity: value.severity, target: value.target, rationale: value.rationale, minimumRecovery: value.minimumRecovery, presentation: value.presentation, ...(verbose ? { baselineEvidence: value.baselineEvidence, candidateEvidence: value.candidateEvidence, checker: value.checker, confidence: value.confidence, provenance: value.provenance } : {}) }; }
