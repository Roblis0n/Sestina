...
function reviewView(summary: CoreReviewSummary | DeterministicReviewResult, verbose: boolean, allFindings: boolean): Readonly<Record<string, unknown>> {
  const selected = allFindings ? summary.findingProjection.raw.map((item) => item.finding) : summary.findingProjection.foreground.map((item) => item.finding);
  const findings = selected.map((item) => findingView(item, verbose));
  const projection = {
    foregroundFindingIds: summary.findingProjection.foreground.map((item) => item.finding.id),
    suppressedFindingIds: summary.findingProjection.suppressed.map((item) => item.findingId),
    omissions: summary.findingProjection.omissions,
    metrics: summary.findingProjection.metrics,
  };
  return { reviewRunId: summary.run.id, episodeId: summary.episode.id, reviewMode: "deterministic_only", semanticStatus: "semantic_pending", reviewReady: summary.outcome.reviewReady, checkerHealth: summary.outcome.checkerHealth, findings, findingCount: summary.run.findings.length, allFindings, findingProjection: projection, ...(verbose ? { coverage: summary.coverage, obligations: summary.obligations, provenance: { inputHash: summary.run.inputHash, snapshotId: summary.run.snapshotId, context: summary.run.context } } : {}) };
}

export async function runReviewCommand(args: ParsedCliArguments, io: CliIo): Promise<CliExitCode> {
  const json = args.options.json === true; const verbose = args.options.verbose === true; const allFindings = args.options["all-findings"] === true; const subcommand = args.positionals[1];
  ...
  try {
    if (subcommand === "run" && args.positionals.length === 3) {
      ...
      const result = reviewView(reviewed.value, verbose, allFindings); const blocking = reviewed.value.run.checkerErrors.length > 0 || reviewed.value.run.findings.some((item) => ["error", "critical"].includes(item.severity) && item.presentation === "foreground");
    ...
    if (subcommand === "show" && args.positionals.length === 3) {
      ...
      const result = reviewView(summary.value, verbose, allFindings); success(io, json, { command: "review show", ...result }, JSON.stringify(result, null, 2)); return EXIT_CODES.success;
    }
