...
## Explicit Sestina handoff

By default, corrections remain ephemeral within the current Host task. Do not
create Sestina records, pin Memory, start a Host connection, or send project
context merely because this Skill runs. A user must explicitly request the
handoff. The public MCP surface is read-only and is not a write transport.

For an authorized handoff, prepare one `ReviewDraftEnvelope` with the selected
project identity, a stable Host invocation identity, the Skill name and version,
the proposed suggestion, and concise public reasons. Include file references
only as bounded source hints; do not read or upload those files on import.
Calculate the envelope hash using Sestina's public canonical envelope contract.
The hash detects changes; it does not confer authority or certify correctness.

Use manual import or a temporary local draft connection explicitly enabled by
the user. Its capability permits only draft submission and that connection's
draft status. Never substitute it for the Research Room user session. Reuse the
same invocation identity when checking an uncertain submission; never create a
new invocation merely to avoid an idempotency conflict. Do not retry submission
automatically. Never place the access token in a draft, log, Receipt, or source
file.

After handoff, return the saved draft identity or an honest uncertain result.
The user continues in Research Room, selects optional context, confirms any
outbound request, and confirms a concrete research effect. A successful draft
submission is not a committed research change, an accepted correction, or
